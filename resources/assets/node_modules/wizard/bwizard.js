...
$.widget("bootstrap.bwizard", {
	...
	options: {
		...
		hideOption: { fade: true },
			...
			// e.g. { blind: true, fade: true, duration: 200}
		showOption: { fade: true, duration: 100 },
	...
	_pageLize: function (init) {
		...
		if (this.list) {
			...
			this.lis.each(function(i){
				...
				$(this).prepend('<span class="label-default">' + (i+1) + '</span>');
				if (!isOL) {
					$(this).find('.label-default').addClass('visible-phone');
				}
			});
		}
	...
	_refreshStep: function () {
		...
		if (this.lis) {
			this.lis.removeClass('active').attr('aria-selected', false).find('.label-default').removeClass('label-danger');
			if (o.activeIndex >= 0 && o.activeIndex <= this.lis.length - 1) {
				if (this.lis) {
					this.lis.eq(o.activeIndex).addClass('active').attr('aria-selected', true).find('.label-default').addClass('label-danger');
				}
			}
		}
	...
	_resetStyle: function ($el) {
		$el.css({ display: '' });
		if (!$.support.opacity) {
			$el[0].style.removeProperty('filter');
		}
	},
