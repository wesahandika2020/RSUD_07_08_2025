					...
					<table class="small__font">
						<thead>
							...
							<tr>
								...
								<td>Umur : <b><?= hitungUmur2($pasien->tanggal_lahir) ?></b></td>
							...
							<tr>
								...
								<td colspan="4">Diagnosis / Masalah Waktu Masuk :
									<b><?= empty($diag_manual) ? ($diagnosa_awal === null ? '' : $diagnosa_awal) : $diag_manual; ?></b>
								</td>
							</tr>
						</thead>
                                        	...
                                        	<?php 	/* if(isset($resume_lab)){
                                        			...
                                        			<?php 	/*} else {
									                        ...
									                        <?php
																...
																foreach ($elementTegr as $c => $d) { $dataR = $d[0]['release']->date; $tanggalR = substr($dataR,6,2); $bulanR = substr($dataR,4,2); $tahunR = substr($dataR,0,4); */ ?>
					...
					<table class="small__font" style="margin-top: 1rem;">
						...
						<tbody>
							<?php foreach ($resume_kontrol as $jk) { ?>
								...
								$hari = empty($jk->hari) ? $dayList[date('D', strtotime($jk->tanggal ?? $jk->tanggal_kontrol))] : $jk->hari;
								?>
								<tr>
									<td align="center"><?= date('d-m-Y', strtotime($jk->tanggal ?? $jk->tanggal_kontrol)); ?></td>
									<td align="center"><?= $hari; ?></td>
									<td align="center"><?= date('H:m', strtotime($jk->tanggal ?? $jk->tanggal_kontrol)); ?></td>
