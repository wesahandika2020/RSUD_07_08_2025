...
class Pemakaian_barang_logistik_model extends CI_Model {
	...
	private function sqlPemakaianBarangLogistik($search)
    {	
    	date_default_timezone_set('Asia/Jakarta');
		$this->db->select("
    ...
    private function _listPemakaianBarangLogistik($limit = 0, $start = 0, $search)
    {
        $this->sqlPemakaianBarangLogistik($search);
        if ($limit !== 0) :
    ...
    function getListDataPemakaianBarangLogistik($limit = 0, $start = 0, $search)
    {
        $result['data'] = $this->_listPemakaianBarangLogistik($limit, $start, $search);
        $result['jumlah'] = $this->sqlPemakaianBarangLogistik($search)->get()->num_rows();
        return $result;
        $this->db->close();
    }

	function simpanDataPemakaianBarangLogistik()
	{
		$this->db->trans_begin();
		$id_barang = safe_post('id_barang');
		$jumlah = safe_post('jumlah');
		foreach ($id_barang as $i => $val) :
			$this->db->select("DISTINCT ON (id_stok) sum(masuk) - sum(keluar) as sisa, id_stok, id_barang, ed ")->from('sm_invrt_stok');
			...
			if (is_array($barang)) :
				foreach ($barang as $j => $row) :
					...
					if ($this->db->trans_status() === false) :
						...
						$result['status'] = false;
						$result['message'] = 'Gagal menyimpan data pemakaian barang logistik';
					endif;
				...
				if ($this->db->trans_status() === false) :
					...
					$result['status'] = true;
					$result['message'] = 'Berhasil menyimpan data pemakaian barang logistik';
				endif;
			endif;
		endforeach;
		return $result;
	...
	function deleteDataPemakaianBarangLogistik($id)
	{
		$this->db->trans_begin();
		$this->db->delete('sm_invrt_stok', ['id' => $id]);
		if ($this->db->trans_status() === false) :
			$this->db->trans_rollback();
			$result['status'] = false;
			$result['message'] = 'Gagal menghapus data pemakaian barang logistik';
		else :
			$this->db->trans_commit();
			$result['status'] = true;
			$result['message'] = 'Berhasil menghapus data pemakaian barang logistik';
		endif;
		return $result;
	}

	function getAutoBarangStokLogistik($search, $start, $limit)
    {
