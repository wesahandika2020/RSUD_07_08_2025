<script>
	...
	function simpanChecklistPenerimaanPasienRawatInap() {
		...
		$.ajax({
			type: 'POST',
			url: '<?= base_url("intensive_care/api/intensive_care/simpan_checklist_penerimaan_pasien_rawat_inap") ?>',
			...
			success: function(data) {
				if (data.status) {
					...
					window.open('<?= base_url('intensive_care/cetak_checklist_penerimaan_pasien_rawat_inap/') ?>' + id, 'Cetak Checklist Penerimaan Pasien Rawat Inap', 'width=' + dWidth + ', height=' + dHeight + ', left=' + x + ', top=' + y);
				} else {
