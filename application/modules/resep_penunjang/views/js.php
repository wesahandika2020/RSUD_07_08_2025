<script>
	...
	function getListResep(p, id_layanan_pendaftaran) {
		...
		$.ajax({
			type: 'GET',
			url: '<?= base_url("resep_penunjang/api/resep_penunjang/get_list_resep_penunjang") ?>/page/' + p + '/jenis/Penunjang/id/' + id,
			...
			success: function(data) {
				...
				$.each(data.data, function(i, v) {
					...
					let detail = v.id + '#' + v.id_pasien + '#' + v.nama + '#' + v.dokter + '#' + v.id_dokter + '#' + hitungUmur(v.tanggal_lahir) + '#' + v.jenis_layanan + '#' + v.id_penjamin + '#' + v.penjamin + '#' + v.cara_bayar + '#' + v.telp + '#resep' + '#penunjang';
		...
		$.ajax({
			...
			success: function(data) {
				...
				$('#modal-form-resep-label').html(`
                    <b>Form Resep Penunjang ${title_mode}</b> | No. RM : ${data.id_pasien}, Nama: ${data.pasien}, 
