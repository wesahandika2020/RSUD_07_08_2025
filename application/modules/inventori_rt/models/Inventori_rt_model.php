...
class Inventori_rt_model extends CI_Model {
    ...
    { 
        $limit = " limit " . $limit . " offset " . $start;
        $where = " and d.id not in (select id_distribusi from sm_penerimaan_distribusi_logistik)";
        if ($param["status"] !== '') :
            $where = "";
        endif;
    ...
    {   
		...
		");
        $this->db->from('sm_invrt_penerimaan as pen');
		$this->db->join('sm_invrt_detail_penerimaan as dpen', 'dpen.id_penerimaan = pen.id');
	...
	function generateNoUrutPenerimaan($jenis = '')
    {
        ...
        $tahun = date('Y');
        $bulan = date('m');
        $row = $this->db->query("select no_penerimaan from sm_invrt_penerimaan where date_part('month', tanggal) = ".$bulan." and date_part('year', tanggal) = ".$tahun." and no_penerimaan is not NULL order by no_penerimaan desc limit 1")->row();
        if ($row) :
    ...
    {   
        ...
        if ($idPenerimaan === '') :
            ...
            $this->db->insert('sm_invrt_penerimaan', $data);
            ...
            $totalBonus = 0;
            $auto = $this->db->query("select max(penerimaan_ke) as penerimaan_ke from sm_invrt_detail_penerimaan where id_penerimaan = '" . (int)$id . "' GROUP BY penerimaan_ke")->row();
            foreach ($idBarang as $key => $value) :
                ...
                $this->db->insert('sm_invrt_detail_penerimaan', $dataDetail);
                ...
                $this->db->insert('sm_invrt_stok', $dataStok);
                ...
                $this->db->update('sm_invrt_stok', $updateDataStok);
            ...
            $this->db->update('sm_invrt_penerimaan', $data);
            if ($this->db->trans_status() === false) :
                $this->db->trans_rollback();
                $result['status'] = false;
            endif;
            $id = $idPenerimaan;
            $this->db->where(['id_penerimaan' => (int)$idPenerimaan])->delete('sm_invrt_detail_penerimaan');
            $this->db->where(['id_transaksi' => (int)$id, 'transaksi' => 'Penerimaan'])->delete('sm_invrt_stok');
    ...
    function getDetailDataPenerimaan($id_penerimaan)
    {
        $sql = "select b.*, p.id_kategori_barang as jenis_penerimaan, 
                ...
                from sm_invrt_penerimaan p
                join sm_invrt_detail_penerimaan dp on (p.id = dp.id_penerimaan)
    ...
    {
        $this->db->trans_begin();
        $this->db->where('id', (int)$id, true)->delete('sm_invrt_penerimaan');
        if ($this->db->trans_status() === false) :
            $this->db->trans_rollback();
            $result['status'] = false;
        endif;
        $this->db->where('id_transaksi', $id, true)->where('transaksi', 'Penerimaan')->delete('sm_invrt_stok');
        if ($this->db->trans_status() === false) :
            $this->db->trans_rollback();
            $result['status'] = false;
