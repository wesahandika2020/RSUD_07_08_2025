<script>
	$(function() {
		...
		$('#btn-add').click(function() {
			resetData()
			$('#modal-koreksi-stok-darah').modal('show')
		})
		...
		$('#barang-auto').select2({
			...
			formatSelection: function(data) {
				$.getJSON('<?= base_url('koreksi_stok_darah/api/koreksi_stok_darah/get_ed_barang') ?>?id_barang=' + data.id, function(data) {
					$('#ed').html('')
					if (data === null) {
		...
		$('#tambahkan').click(function() {
			...
			entriNewTableRow(id_barang, nama_barang, ed, sisa, kenyataan, penyesuaian, alasan)
		})
		...
		$('#ed').change(function() {
			...
			$.ajax({
				type: 'GET',
				url: '<?php echo base_url('koreksi_stok_darah/api/koreksi_stok_darah/sisa_barang_koreksi_stok') ?>',
	...
	function getListKoreksiStokDarah(page, id_koreksi_stok) {
		var id = '';
		...
		$.ajax({
			type: 'GET',
			url: '<?php echo base_url('koreksi_stok_darah/api/koreksi_stok_darah/get_list_koreksi_stok_darah/page/') ?>' + page + '/id/' + id,
			...
			success: function(data) {
				if ((page > 1) & (data.data.length === 0)) {
					getListKoreksiStokDarah(page - 1)
					return false;
				}
	...
	function paging(page) {
		getListKoreksiStokDarah(page)
	}

	function resetData() {
		$('#table-list tbody').empty()
		$('#form-koreksi-stok-darah')[0].reset()
	...
	function entriNewTableRow(id_barang, nama_barang, ed, sisa, kenyataan, penyesuaian, alasan) {
		if (id_barang === '') {
		...
		$('#barang-auto, #ed, #sisa, #kenyataan, #penyesuaian, #alasan').val('')
		$('#s2id_barang-auto a .select2-chosen').html('Pilih Barang...')
	}
	...
	function simpanKoreksiStokDarah() {
		let jml = $('.rows').length
		if (jml === 0) {
			...
			buttons: {
				...
				konfirmasi: {
					...
					callback: function() {
						...
						$.ajax({
							type: 'POST',
							url: '<?php echo base_url('koreksi_stok_darah/api/koreksi_stok_darah/simpan_koreksi_stok_darah') ?>',
							data: $('#form-koreksi-stok-darah').serialize(),
							beforeSend: function() {
								showLoader()
							},
							success: function(data) {
								if (data.status) {
									...
									getListKoreksiStokDarah(1)
									$('#modal-koreksi-stok-darah').modal('hide')
								}
							},
							complete: function() {
								hideLoader()
	...
	function cetakKoreksiStokDarah() {
		...
		window.open('<?php echo base_url() ?>koreksi_stok_darah/printing_koreksi_stok_darah/' + kategori, 'Cetak Lembar Koreksi Stok', 'width=' + dWidth + ', height=' + dHeight + ', left=' + x + ',top=' + y)
	}

	function cetakExcelKoreksiStokDarah() {
		location.href = '<?php echo base_url('koreksi_stok_darah/export_koreksi_stok_darah') ?>?' + $('#form-search').serialize()
	}
</script>