<script>
	$(function() {
        ...
        $('#btn-tambah-data').click(function() {
            ...
            $('#modal-pendaftaran-label').html('Form Entri Pendaftaran Radiologi');
		});
	...
	function getListPemeriksaan(page, id) {
        ...
        $.ajax({
            type: 'GET',
            url: '<?= base_url('pendaftaran/api/pendaftaran/get_list_pendaftaran') ?>/page/' + page + '/jenis_pendaftaran/Radiologi',
    ...
    function konfirmasiSimpanDataPendaftaran() {
		...
		klik = null;
		swal.fire({
			title: 'Simpan Pendaftaran Radiologi',
			text: "Apakah anda yakin akan melakukan entri pendaftaran Radiologi ?",
			icon: 'question',
    ...
    function cetakLabelBerkas(id, id_pasien, id_layanan_pendaftaran, size) {
        ...
        window.open('<?= base_url('pendaftaran_rad/cetak_label_berkas/') ?>' + id + '/' + '\'' + id_pasien + '\'' + '/' + id_layanan_pendaftaran, 'Cetak Label Berkas', 'width=' + dWidth + ', height=' + dHeight + ', left=' + x + ', top=' + y);
    }
    ...
    function cetakKartuPasien(id, id_pasien) {
        ...
        window.open('<?= base_url('pendaftaran_rad/cetak_kartu_pasien_rad/') ?>' + id, 'Cetak Kartu Pasien', 'width=' + dWidth + ', height=' + dHeight + ', left=' + x + ', top=' + y);
    }
    ...
    function cetakBuktiKunjungan(id) {
        ...
        window.open('<?= base_url('pendaftaran_rad/cetak_bukti_kunjungan_rad/') ?>' + id, 'Cetak Bukti Kunjungan', 'width=' + dWidth + ', height=' + dHeight + ', left=' + x + ', top=' + y);
    }
                ...
                if (data.pasien !== null) {
                    ...
                    if (data.layanan.length > 0) {
                        $.each(data.layanan, function(i, v){
                            if (i === 0) {
                                $('#id-layanan').val(v.id);
                                $('#id-daftar-layanan').val(v.id);
                                $('#cara-bayar-detail').html(v.cara_bayar);
                                $('#jenis-layanan-detail').html(v.jenis_layanan);

                                $('#dokter_rujuk_rad').val(v.id_dokter);
                                $('#s2id_dokter_rujuk_rad a .select2-chosen').html(v.dokter);
                            };
                        });

                        if ($('#id-layanan').val() !== '') {
                            get_pemeriksaan_rad($('#id-layanan').val());    
                        };

                        $('#modal-detail-pendaftaran').modal('show');
                        $('#modal-detail-pendaftaran-label').html('Detail Pendaftaran Radiologi');
                    }       
