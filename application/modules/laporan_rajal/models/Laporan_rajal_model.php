...
class Laporan_rajal_model extends CI_Model
{
    ...
    public function list_laporan()
    {
        return array(
            ...
            '8'     => 'Laporan Pasien Menerima Resep',
        );
    }
    ...
    public function getListDataLaporanKunjungan($limit, $start, $search)
    {
		...
		if ($search["status_kunjungan"] !== "") {
            $param .= " AND pd.status = '" . $search["status_kunjungan"] . "' ";
        }

        if ($search["status_terlayani"] !== "") {
            $param .= " AND lp.status_terlayani = '" . $search["status_terlayani"] . "' ";
        }
        ...
        $select = "SELECT pd.ID,
                        ...
                        lp.status_terlayani,
                        lp.tindak_lanjut,
                        pg.nama AS nama_dokter, pd.status AS status_kunjungan";
    ...
    private function sqlDataDiagnosa($id)
    {
        ...
        $this->db->select("d.*, pg.nama nama_dokter, gss.nama as diagnosa, null as nama_poli, p.jenis_rawat, ps.nama as nama_pasien, p.id as id_pendaftaran, DATE_PART('year', age(now(), ps.tanggal_lahir)) AS umur, ps.alamat, ps.kelamin, pj.nama as cara_bayar, lp.tindak_lanjut, b.nama as nama_bangsal, ps.id as no_rm", false);
        ...
        return $this->db->group_by('p.id, d.id, gss.nama, ps.nama, ps.tanggal_lahir, ps.alamat, ps.kelamin, pj.nama, lp.tindak_lanjut, b.nama, ps.id, pg.nama', 'asc');
    ...
    function getListDataLaporanDiagnosaTr($limit, $start, $search)
    {
        $param = "";
		if ($search["periode_laporan"] === "Harian") {
			if ($search["tanggal_harian"] !== "") :
        ...
        $sql = "
            SELECT DISTINCT ON (lp.id) 
                ...
                p.tanggal_lahir, 
                pj.nama AS penjamin, 
                spg.nama AS dokter_dpjp, 
                d.id_pengkodean, 
                d.id_golongan_sebab_sakit, 
                gss.kode_icdx,
                gss.nama AS nama_golongan_sebab_sakit,
                d.diagnosa_manual, 
            ...
            LEFT JOIN sm_tenaga_medis AS tm ON tm.id = lp.id_dokter
            LEFT JOIN sm_pegawai AS spg ON spg.id = tm.id_pegawai
            JOIN sm_spesialisasi AS sp ON sp.id = lp.id_unit_layanan
            WHERE pjl.id_resep IS NOT NULL 
            " . $param . "
            ORDER BY lp.id, pd.tanggal_daftar DESC
        ";
        ...
        $count = "
            ...
            FROM (
                ...
                LEFT JOIN sm_penjualan as pjl ON pjl.id_layanan_pendaftaran = lp.id
                LEFT JOIN sm_tenaga_medis as tm ON tm.id = lp.id_dokter
                LEFT JOIN sm_pegawai as sp ON sp.id = tm.id_pegawai
                WHERE pjl.id_resep IS NOT NULL 
                " . $param . "
            ) AS total_count
        ";
        ...
        foreach ($query as $value) {
			$value->resep_data = $this->getResepListLaporanByIdLayananPendaftaran($value->id_layanan_pendaftaran);
		}

        $data["periode"] = $periode;
		$data["data"]    = $query;
		$data["jumlah"]  = $this->db->query($sql)->num_rows();
		//$data["jumlah"]  = $this->db->query($count)->row()->count;

        return $data;
    }

    function getExportDataLaporanDiagnosaTr($search)
    {
        $param = "";
		if ($search["periode_laporan"] === "Harian") {
			if ($search["tanggal_harian"] !== "") :
        ...
        $sql = "
            ...
            WHERE pjl.id_resep IS NOT NULL
            " . $param . "
            ORDER BY lp.id, pd.tanggal_daftar
        ";
    ...
    function getResepListLaporanByIdLayananPendaftaran($id_layanan_pendaftaran)
	{
		$sql = "
            SELECT
                lp.id as id_pelayanan,
                pd.id as id_pendaftaran,
                r.id as id_resep,
                r.tanggal_resep as waktu_penjualan,
                rt.id as no_resep,
                rtr.id as id_resep_tebus_r,
                rtrd.id as id_resep_tebus_r_detail,
                b.nama_lengkap as nama_obat,
                rtrd.dosis_racik as dosis,
                st.nama as nama_satuan,
                rtrd.jumlah_pakai as jumlah_pakai

            FROM sm_pendaftaran as pd
            JOIN sm_layanan_pendaftaran lp ON pd.id = lp.id_pendaftaran
            LEFT JOIN sm_resep as r on r.id_layanan_pendaftaran = lp.id
            LEFT JOIN sm_resep_tebus as rt on rt.id_resep = r.id
            LEFT JOIN sm_resep_tebus_r as rtr on rtr.id_resep_tebus = rt.id
            LEFT JOIN sm_resep_tebus_r_detail as rtrd on rtrd.id_resep_tebus_r = rtr.id
            LEFT JOIN sm_barang as b on b.id = rtrd.id_barang
            LEFT JOIN sm_satuan as st on st.id = b.satuan_kekuatan
            WHERE 
                lp.id = " . $id_layanan_pendaftaran . "
            ORDER BY 
