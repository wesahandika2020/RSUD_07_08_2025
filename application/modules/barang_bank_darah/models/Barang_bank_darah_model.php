...
class Barang_bank_darah_model extends CI_Model
{
    ...
    private function _list($limit = 0, $start = 0, $search)
    {
        $this->sql($search);
        if ($limit !== 0) :
    ...
    function getListData($start, $limit, $search)
    {
        ...
        endforeach;
        foreach ($result as $key => $value) :
            $this->db->select("COALESCE(SUM ( s.masuk ) - SUM ( s.keluar ), 0) AS sisa")
                ->from('sm_stok_bank_darah AS s')
                ->where('s.id_barang', $value->id, true)
                ->where('s.ed > now()::date');
            $result[$key]->stok_tersedia = $this->db->get()->row()->sisa;
        endforeach;
        $result['data'] = $result;
    ...
    function simpanDataBarang()
    {
        ...
        $golongan_darah = safe_post("golongan_darah_barang");
        $rhesus = safe_post("rhesus_barang");
        ...
        if ($id === "") :
            ...
            $this->db->update("sm_kemasan_barang", array("default_kemasan" => "0"));
            $get = $this->db->query("select id, (isi*isi_satuan) as jml from sm_kemasan_barang where id_barang = '" . $id . "' order by jml asc limit 1")->row();
            $update = "update sm_kemasan_barang set default_kemasan = '1' where id = '" . $get->id . "'";
            $this->db->query($update);
        endif;
