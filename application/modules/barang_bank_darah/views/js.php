...
<script>
    $(function() {
        ...
        $('#btn-tambah').click(function() {
            $('#modal-barang-bank-darah').modal('show');
            $('#modal-barang-bank-darah-label').html('Tambah Barang Darah');
            resetForm();
        ...
        $('#btn-excel').click(function() {
            location.href='<?= base_url('barang_bank_darah/export_data_barang') ?>?'+$('#form-search').serialize()+'&keyword='+$('#pencarian').val();
        });
        ...
        var html = /* html */ `
            <tr class="mother mb-1" id="mother${i}" style="height:37px;">
                <td width="3%" class="center v-center">
                    <input type="radio" name="default" value="${i}" checked title="Kemasan Jual Default" id="default${i}">
                </td>
                <td width="70%">
            ...
            buttons: {
                ...
                hapus: {
                    ...
                    callback: function() {
                        ...
                        $.ajax({
                            type : 'DELETE',
                            url: '<?= base_url("barang_bank_darah/api/barang_bank_darah/kemasan_barang") ?>/id/'+id,
                            cache: false,
                            dataType : 'JSON',
    ...
    function konfirmasiSimpanData() {
        ...
        if ($('#golongan-darah-barang').val() === '') {
            syams_validation('#golongan-darah-barang', 'Kolom golongan darah barang harus diisi.');
            $('#wizard-form').bwizard('show', '0'); return false;
        }

        if ($('#rhesus-barang').val() === '') {
            syams_validation('#rhesus-barang', 'Kolom rhesus barang harus diisi.');
            $('#wizard-form').bwizard('show', '0'); return false;
        }
    ...
    function simpanData() {
        ...
        $.ajax({
            type: 'POST',
            url: '<?= base_url("barang_bank_darah/api/barang_bank_darah/simpan_barang") ?>/' + update,
            data: $('#form-barang-bank-darah').serialize(),
            cache: false,
            dataType: 'JSON',
            beforeSend: function() {
                showLoader();
            ...
            success: function(data) {
                ...
                $('#modal-barang-bank-darah').modal('hide');
    ...
    function getListBarang(p, id_barang) {
        ...
        $.ajax({
            type: 'GET',
            url: '<?= base_url("barang_bank_darah/api/barang_bank_darah/get_list_barang") ?>/page/' + p + '/id/' + id,
                ...
                $.each(data.data, function(i, v) {
                    ...
                    let html = /* html */ `
                        <tr ${redBlock}>
                            ...
                            <td>${v.code}</td>
                            <td>${v.nama}</td>
                            <td class="center">${v.golongan_darah}</td>
                            <td class="center">${(v.rhesus === '+' ? 'Positif (+)' : 'Negative (-)')}</td>
                            <td class="right">${money_format(v.hpp)}</td>
                            <td class="center" style="white-space: nowrap;">${number_format(v.stok_tersedia, 0, ',', '.')}</td>
    ...
    function editBarang(id) {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                $('#modal-barang-bank-darah').modal('show');
                $('#modal-barang-bank-darah-label').html('Edit Barang');
            },
            error: function(e) {
                accessFailed(e.status);
            }
