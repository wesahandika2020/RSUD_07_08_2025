...
<div id="permintaan-logistik-search" class="modal fade" data-backdrop="static" role="dialog" aria-labelledby="permintaan-logistik-search-label" aria-hidden="true">
	<div class="modal-dialog" style="max-width:500px">
		<div class="modal-content">
			<div class="modal-header">
				<h4 class="modal-title" id="permintaan-logistik-search-label">Form Parameter Laporan</h4>
				<button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
			</div>
			<div class="modal-body">
				<?= form_open('', 'id=form-search-permintaan-logistik role=form class=form-horizontal') ?>
				...
				<div class="form-group row tight">
					<label for="pembayaran_permintaan" class="col-3 col-form-label">Pembayaran</label>
					<div class="col-md-9">
						<?= form_dropdown('pembayaran_permintaan', $pembayaran, array(), 'id=pembayaran_permintaan class=form-control') ?>
					</div>
				</div>
				<div class="form-group row tight">
				...
				<div class="form-group row tight">
					<label for="barang-search-logistik" class="col-md-3 col-form-label">Barang</label>
					<div class="col-md-9">
						<input type="text" name="barang" id="barang-search-logistik" class="select2-input" placeholder="Semua Barang">
					</div>
				</div>
				<?= form_close() ?>
			</div>
			<div class="modal-footer">
				...
				<button type="button" class="btn btn-info waves-effect" onclick="getLaporanPermintaanLogistik(1)"><i class="fas fa-paper-plane mr-1"></i> Tampilkan</button>
			</div>
		</div>
		<!-- /.modal-content -->
	</div>
...
<script>
	...
	function resetFormPermintaanlogistik() {
		<?php date_default_timezone_set('Asia/Jakarta'); ?>
		...
		$('#kategori-permintaan, #jenis-pasien, #pembayaran_permintaan').val('')
		$('.harian-permintaan, #tanggal-harian-permintaan, .rentang-waktu-permintaan').hide()
	}

	function getLaporanPermintaanLogistik(page) {
		//Laporan 01
		$('#page-now').val(page)
		...
		if ($('#jenis-laporan').val() == "4") {
			$('.lap-permintaan-logistik, #table-lap-permintaan-logistik tbody').show()

			$.ajax({
				type: 'GET',
				url: '<?= base_url('laporan_inventori_logistik/api/laporan_inventori_logistik/get_list_laporan_permintaan') ?>/page/' + page + '/jenis/',
				data: $('#form-search-permintaan-logistik').serialize(),
				cache: false,
				dataType: 'JSON',
				beforeSend: function() {
					showLoader()
				...
				success: function(data) {
					if ((page - 1) & (data.data.length === 0)) {
						getLaporanPermintaanLogistik(page - 1);
						return false;
					}

					$('#jenis-periode-permintaan-logistik').html('Periode : ' + data.periode);
					...
					$.each(data.data, function(i, v) {
						...
						let periode = '';
						if ($('#jenis-periode-permintaan-logistik').val() == 'Harian') {
							periode = 'Laporan Harian';
						}
						if ($('#jenis-periode-permintaan-logistik').val() == 'Bulanan') {
							periode = 'Laporan Bulanan';
						}
						if ($('#jenis-periode-permintaan-logistik').val() == 'Rentang Waktu') {
						...
						$('#table-lap-permintaan-logistik tbody').append(html);
					})
				},
				complete: function() {
					hideLoader()
	...
	$('#barang-search-logistik').select2({
		ajax: {
