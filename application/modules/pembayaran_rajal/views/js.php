<script>
	...
	function getListPembayaran(page) {
		...
		$.ajax({
			type: 'GET',
			url: '<?= base_url("keuangan/api/keuangan/get_list_rekap_billing") ?>/page/' + page + '/jenis/rawat_jalan',
			...
			success: function(data) {
				...
				$.each(data.data, function(i, v) {
					...
					let status_kunjungan = '';
					if (v.status_kunjungan === 'Aktif') {
                        status_kunjungan = '<h5><span class="label label-success"><i class="fas fa-spinner fa-spin mr-1"></i>Aktif</span></h5>';
                    } else {
                        status_kunjungan = '<h5><span class="label label-danger"><i class="fas fa-stop mr-1"></i>Pulang</span></h5>';
                    }
					...
					let button = '';
					const excludeIdPenjamin = ['1', '9'];

					if(!excludeIdPenjamin.includes(v.id_penjamin)){
						button = `<button type="button" class="btn btn-xs ${v.konfirmasi_kasir === '1' ? 'btn-success' : 'btn-secondary'}" title="Klik untuk melakukan pembayaran" onclick="konfirmasiKasir(${v.id})"><i class="fas fa-check"></i></button>`
					}

					buttonBayar = ``;
					if ('<?= $this->session->userdata("account_group") ?>' !== 'Casemix') {
						buttonBayar = `<button type="button" class="btn btn-secondary btn-xs" title="Klik untuk melakukan pembayaran" onclick="inputPembayaran(${v.id}, '${((v.cara_bayar !== 'Asuransi' ? v.cara_bayar : v.penjamin))}', '${v.jenis_layanan}')"><i class="fas fa-money-bill-alt mr-1"></i>Bayar</button>`;
					}
					...
					let html = /* html */ `
						<tr>
							...
							<td class="center">${status_kunjungan}</td>
