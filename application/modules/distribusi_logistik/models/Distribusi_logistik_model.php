...
class Distribusi_logistik_model extends CI_Model {
    ...
    private function _listDistribusi($limit, $start, $search)
    {
		...
		foreach ($result as $i => $value) :
			...
			$this->db->from('sm_invrt_detail_distribusi as dd');
			$this->db->join('sm_kemasan_barang as kb', 'kb.id = dd.id_kemasan_barang');
	...
	function generateKodeDistribusi()
    {
        ...
        $tahun = date('Y');
        $bulan = date('m');
        $row = $this->db->query("select kode_dikirim from sm_invrt_distribusi where date_part('month', tanggal_dikirim) = ".$bulan." and date_part('year', tanggal_dikirim) = ".$tahun." and kode_dikirim is not NULL order by kode_dikirim desc limit 1")->row();
        if ($row) :
	...
	function simpanDataDistribusi() 
	{
		...
		$unit_kirim = $this->db->select('id_unit_asal')->from('sm_invrt_distribusi')->where('id', (int)$id_distribusi)->get()->row();
		...
		$this->db->where('id', $id_distribusi, true)->update('sm_invrt_distribusi', $data);
		...
		foreach ($id_detail_distribusi as $i => $data) :
			...
			$this->db->where('id', (int)$data, true)->update('sm_invrt_detail_distribusi', $data_detail);
			...
			$stok = $this->db->query("select sum(masuk) - sum(keluar) as sisa, id_stok, id_barang, ed from sm_invrt_stok where id_barang = '".$id_barang[$i]."' and id_unit = '".$this->unit."' group by id_stok, id_barang, ed having sum(masuk) - sum(keluar) > 0 order by id_stok asc")->result();
			...
			if (is_array($stok)) :
				foreach ($stok as $i => $val) :
					...
					$this->db->insert('sm_invrt_stok', $data_stok);
					if ($this->db->trans_status() === false) :
						$this->db->trans_rollback();
						$result['status'] = false;
					endif;
