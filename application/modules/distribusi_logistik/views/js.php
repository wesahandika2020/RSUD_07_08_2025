<script>
	$(function() {
		...
		$('#tanggal-awal-minta, #tanggal-akhir-minta, #tanggal-awal-kirim, #tanggal-akhir-kirim').datepicker({
            format: 'dd/mm/yyyy'
	...
	function reloadData() {
		...
		$('#tanggal-awal-minta').val('')
		$('#tanggal-akhir-minta').val('<?= date('d/m/Y') ?>')
		$('#tanggal-awal-kirim').val('')
		$('#tanggal-akhir-kirim').val('')
	}
	...
	function getListDistribusi(page) {
		...
		$.ajax({
			type: 'GET',
			url: '<?php echo base_url('distribusi_logistik/api/distribusi_logistik/get_list_distribusi_logistik') ?>/page/' + page,
	...
	function distribusiPermintaan(detail) {
		...
		$.ajax({
			type: 'GET',
			url: '<?php echo base_url('permintaan_logistik/api/permintaan_logistik/get_permintaan_logistik') ?>/id/' + data[0],
			cache: false,
			dataType: 'JSON',
			beforeSend: function() {
				showLoader()
	...
	function simpanDistribusi(page) {
		$.ajax({
			type: 'POST',
			url: '<?php echo base_url('distribusi_logistik/api/distribusi_logistik/simpan_distribusi_logistik') ?>',
	...
	function printDistribusi(id) {
		...
		window.open('<?= base_url() ?>distribusi_logistik/printing_bukti_distribusi?id=' + id,'Cetak Bukti Distribusi Barang','width='+dWidth+', height='+dHeight+', left='+x+',top='+y)
	}
</script>