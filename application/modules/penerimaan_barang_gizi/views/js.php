<script>
	...
	$(function() {
		...
		$('#btn-add').click(function() {
			reloadData()
			$('#modal-penerimaan-title').html('Form Penerimaan Barang Gudang Gizi')
	...
	function getListPenerimaan(page, id) {
		...
		$.ajax({
			type: 'GET',
			url: '<?= base_url("inventori_gizi/api/inventori_gizi/get_list_penerimaan") ?>/page/' + page + '/id/' + id,
			data: $('#form-search').serialize() + '&jenis_barang=Gizi',
			cache: false,
			dataType: 'JSON',
			beforeSend: function() {
				showLoader()
	...
	function printPenerimaan(id_penerimaan) {
		window.open('<?= base_url() ?>inventori_gizi/printing_penerimaan_gizi/' + id_penerimaan, 'Cetak Penerimaan','width='+dWidth+', height='+dHeight+', left='+x+',top='+y)
	}
	...
	function deletePenerimaan(id, page, string) {
		...
		swal.fire({
			...
			if (result.value) {
				$.ajax({
					type: 'DELETE',
					url: '<?= base_url("inventori_gizi/api/inventori_gizi/delete_penerimaan") ?>' + '/id/' + id,
					cache: false,
					dataType: 'JSON',
					beforeSend: function() {
						showLoader()
