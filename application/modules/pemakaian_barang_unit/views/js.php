<script>
	$(function() {
		getListPemakaianBarangUnit(1)
		
		$('#btn-add').click(function() {
			resetData()
			$('#modal-pemakaian-barang-unit').modal('show')
		})
	...
	function getListPemakaianBarangUnit(page) {
		$('#page-now').val(page)
		$.ajax({
			type:'GET',
			url: '<?php echo base_url('pemakaian_barang_unit/api/pemakaian_barang_unit/get_list_pemakaian_barang_unit/page/') ?>' + page,
			...
			success: function(data) {
				if ((page - 1) & (data.data.length === 0)) {
					getListPemakaianBarangUnit(page - 1)
					return false;
				}
				...
				$.each(data.data, function(i, val) {
					let html = /* html */ `
						<tr>
							...
							<td class="center">${datefmysql(val.ed)}</td>
							<td class="center"><i>${val.nama_user}</i></td>
							<td class="right wrapper">
								<button type="button" class="btn btn-secondary btn-xs" onclick="deletePemakaianBarangUnit(${val.id}, ${data.page})"><i class="fas fa-trash-alt mr-1"></i>Hapus</button>
							</td>
						</tr>
					`;
	...
	function paging(page) {
		getListPemakaianBarangUnit(page)
	}
	...
	function simpanPemakaianBarangUnit() {
		...
		swal.fire({
			title: 'Konfirmasi Simpan',
 			html: "Apakah anda yakin akan menyimpan data pemakaian barang unit ini ?",
			icon: 'question',
			...
			if (result.value) {
				$.ajax({
					type: 'POST',
					url: '<?= base_url("pemakaian_barang_unit/api/pemakaian_barang_unit/simpan_pemakaian_barang_unit") ?>',
					data: $('#form-pemakaian-barang-unit').serialize(),
					cache: false,
					dataType: 'JSON',
					beforeSend: function() {
						showLoader()
					...
					success: function(data) {
						...
						if (data.status) {
							type = 'Success';
							getListPemakaianBarangUnit(1)
							$('#modal-pemakaian-barang-unit').modal('hide')
						}
					...
					error: function(e) {
						messageCustom('Terjadi Kesalahan, Gagal menyimpan data pemakaian barang unit', 'Error')
					},
				})
			}
		})
	...
	function deletePemakaianBarangUnit(id, page) {
		swal.fire({
			title: 'Konfirmasi Hapus',
 			html: "Apakah anda yakin akan menghapus data pemakaian barang unit ini ?",
			icon: 'question',
			showCancelButton: true,
			buttonsStyling: true,
			...
			if (result.value) {
				$.ajax({
					type: 'DELETE',
					url: '<?= base_url("pemakaian_barang_unit/api/pemakaian_barang_unit/delete_pemakaian_barang_unit/id/") ?>' + id,
					cache: false,
					dataType: 'JSON',
					beforeSend: function() {
						showLoader()
					...
					error: function(e) {
						messageCustom('Terjadi Kesalahan, Gagal menghapus data pemakaian barang unit', 'Error')
					},
				})
			}
		})
