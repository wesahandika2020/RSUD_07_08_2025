...
class Permintaan_gizi_model extends CI_Model {
    ...
    {
        $this->db->select("u.id ,u.nama", false);
        $this->db->from('sm_unit as u');
        $this->db->where_in('u.id', ['346', '261']); // Menggunakan where_in untuk ID 332 atau 261

        $result = $this->db->get()->result(); // Simpan hasil query ke variabel
        $this->db->close(); // Tutup koneksi setelah query selesai

        return $result; // Kembalikan hasil query
    }
    

	private function sqlGiziUnitSisaStok($search)
    {
    ...
    private function _listGiziUnitSisaStok($limit = 0, $start = 0, $search)
    {
        $this->sqlGiziUnitSisaStok($search);
        if ($limit !== 0) :
        ...
        foreach ($result as $i => $value) :
            ...
            $result[$i]->sisa = $this->db->select("COALESCE(sum(masuk) - sum(keluar), 0.00) as sisa")
                                ->from('sm_gizi_stok')->where('id_barang', $value->id)->where('id_unit', $search['unit'])
                                ->get()->row()->sisa;
            // query cari kemasan
    ...
    function getListDataGiziUnitSisaStok($limit = 0, $start = 0, $search)
    {
        $result['data'] = $this->_listGiziUnitSisaStok($limit, $start, $search);
        $result['jumlah'] = $this->sqlGiziUnitSisaStok($search)->get()->num_rows();
        return $result;

        $this->db->close();
    }

    function generateKodePermintaanGizi()
    {
        date_default_timezone_set('Asia/Jakarta');
        $tahun = date('Y');
        $bulan = date('m');
        $row = $this->db->query("select kode from sm_gizi_distribusi where date_part('month', tanggal_permintaan) = ".$bulan." and date_part('year', tanggal_permintaan) = ".$tahun." and kode is not NULL order by kode desc limit 1")->row();
        if ($row) :
    ...
    function simpanDataPermintaanGizi()
    {
        $this->db->trans_begin();
        $id = safe_post('id');
        ...
        if ($id === '') :
            $data = array(
                ...
                'kode' => $this->generateKodePermintaanGizi(),
            ...
            $this->db->insert('sm_gizi_distribusi', $data);
            ...
            if (is_array($kemasan)) :
                foreach ($kemasan as $i => $val) :
                    ...
                    $this->db->insert('sm_gizi_detail_distribusi', $data_detail);
                endforeach;
            endif;
            ...
            if ($this->db->trans_status() === false) :
                ...
                $result['status'] = false;
                $result['message'] = 'Gagal menyimpan data permintaan gizi';
            else :
                $this->db->trans_commit();
                $result['status'] = true;
                $result['message'] = 'Berhasil menyimpan data permintaan gizi';
            endif;
            $result['action'] = 'add';
        else :
            $data = array(
            ...
            $this->db->where('id', (int)$id, true)->update('sm_gizi_distribusi', $data);
            $id_distribusi = $id;
            $this->db->where('id_distribusi', (int)$id_distribusi, true)->delete('sm_gizi_detail_distribusi');
            if (is_array($kemasan)) :
                foreach ($kemasan as $i => $val) :
                    $sql = "select b.hna, b.hpp from sm_barang as b
                            join sm_kemasan_barang as kb on (kb.id_barang = b.id) 
            ...
            if ($this->db->trans_status() === false) :
                $this->db->trans_rollback();
                $result['status'] = false;
                $result['message'] = 'Gagal mengubah data permintaan gizi';
            else :
                $this->db->trans_commit();
                $result['status'] = true;
                $result['message'] = 'Berhasil mengubah data permintaan gizi';
            endif;
    ...
    private function sqlPermintaanGizi($search)
    {
        date_default_timezone_set('Asia/Jakarta');
		...
		$this->db->from('sm_gizi_distribusi as d');
		$this->db->join('sm_gizi_detail_distribusi as dd', 'dd.id_distribusi = d.id');
    ...
    private function _listPermintaanGizi($limit = 0, $start = 0, $search)
    {
        $this->sqlPermintaanGizi($search);
        if ($limit !== 0) :
            $this->db->limit($limit, $start);
        endif;
    ...
    function getListDataPermintaanGizi($limit = 0, $start = 0, $search)
    {
        $result['data'] = $this->_listPermintaanGizi($limit, $start, $search);
        $result['jumlah'] = $this->sqlPermintaanGizi($search)->get()->num_rows();
        return $result;
        $this->db->close();
    }

    function getDataPermintaanGizi($id)
    {
        $param = NULL;
        date_default_timezone_set('Asia/Jakarta');
        $add_select = NULL;
        if (isset($id) && $id !== '') :
            $param .= " and d.id = '".$id."'";
            $add_select = ", 
                COALESCE((select sum(stk.masuk) - sum(stk.keluar) from sm_gizi_stok as stk where stk.id_barang = b.id and stk.id_unit = '".$this->session->userdata('id_unit')."'), 0.00) as sisa,
                ...
                COALESCE((select sum(stk.masuk) - sum(stk.keluar) from sm_gizi_stok as stk where stk.id_barang = b.id and stk.id_unit = u.id), 0.00) as sisa_asal ";
        endif;
        ...
        $sql = "from sm_gizi_distribusi as d 
                join sm_gizi_detail_distribusi as dd on (dd.id_distribusi = d.id) 
    ...
    function deleteDataPenerimaanGizi($id)
    {
        $this->db->trans_begin();
        $this->db->where('id', $id, true)->delete('sm_gizi_distribusi');
        if ($this->db->trans_status() === false) :
            $this->db->trans_rollback();
            $result['status'] = false;
            $result['message'] = "Gagal menghapus data penerimaan gizi!";
        else :
            $this->db->trans_commit();
            $result['status'] = true;
            $result['message'] = "Berhasil menghapus data penerimaan gizi!";
        endif;
        return $result;
    }
}