	...
	<div class="modal-dialog" style="max-width:500px">
		<div class="modal-content">
			...
			<div class="modal-footer">
				...
				<button type="button" class="btn btn-info waves-effect" onclick="getLaporanPembelianGizi(1)"><i class="fas fa-paper-plane mr-1"></i> Tampilkan</button>
			</div>
		</div>
		<!-- /.modal-content -->
	</div>
...
<script>
	...
	function resetFormPembeliangizi() {
	...
	function getLaporanPembelianGizi(page) {
		
		$('#page-now').val(page)
		openData();

		if ($('#jenis-laporan').val() == "3") {
			$('.lap-pembelian-gizi, #table-lap-pembelian-gizi tbody').show()

			$.ajax({
				type: 'GET',
				url: '<?= base_url('laporan_inventori_gizi/api/laporan_inventori_gizi/get_list_laporan_pembelian') ?>/page/' + page + '/jenis/',
				...
				success: function(data) {
					...
					if ((page - 1) & (data.data.length === 0)) {
						getLaporanPembelianGizi(page - 1);
						return false;
					}
					...
					$.each(data.data, function(i, v) {
						...
						$('#table-lap-pembelian-gizi tbody').append(html);
