...
class Distribusi_gizi_model extends CI_Model {
	...
	private function sqlDistribusi($search)
    {
		...
		$this->db->where('d.id_unit_tujuan', $this->session->userdata('id_unit'), true);
		...
		if (isset($search['tanggal_awal_kirim']) && isset($search['tanggal_akhir_kirim'])) :
			if (($search['tanggal_awal_kirim'] !== '') & ($search['tanggal_akhir_kirim'] !== '')) :
				$this->db->where("d.tanggal_dikirim BETWEEN '" . date2mysql($search['tanggal_awal_kirim']) . "' AND '" . date2mysql($search['tanggal_akhir_kirim']) . "'");
			endif;
		endif;
    ...
    private function _listDistribusi($limit, $start, $search)
    {
		...
		foreach ($result as $i => $value) :
			...
			$this->db->from('sm_gizi_detail_distribusi as dd');
			$this->db->join('sm_kemasan_barang as kb', 'kb.id = dd.id_kemasan_barang');
	...
	function generateKodeDistribusi()
    {
        ...
        $tahun = date('Y');
        $bulan = date('m');
        $row = $this->db->query("select kode_dikirim from sm_gizi_distribusi where date_part('month', tanggal_dikirim) = ".$bulan." and date_part('year', tanggal_dikirim) = ".$tahun." and kode_dikirim is not NULL order by kode_dikirim desc limit 1")->row();
        if ($row) :
	...
	function simpanDataDistribusi() 
	{
		...
		$unit_kirim = $this->db->select('id_unit_asal')->from('sm_gizi_distribusi')->where('id', (int)$id_distribusi)->get()->row();
		$ket_unit = $this->inventori->namaUnit($unit_kirim->id_unit_asal);
		...
		$this->db->where('id', $id_distribusi, true)->update('sm_gizi_distribusi', $data);
		...
		foreach ($id_detail_distribusi as $i => $data) :
			...
			$data_detail = array(
				'jumlah_dikirim' => (float)$jumlah_dikirim[$i],
				'hna' => (float)$barang->hna,
				'hpp' => (float)$barang->hpp,
			);
			$this->db->where('id', (int)$data, true)->update('sm_gizi_detail_distribusi', $data_detail);
			...
			$stok = $this->db->query("select sum(masuk) - sum(keluar) as sisa, id_stok, id_barang, ed from sm_gizi_stok where id_barang = '".$id_barang[$i]."' and id_unit = '".$this->unit."' group by id_stok, id_barang, ed having sum(masuk) - sum(keluar) > 0 order by id_stok asc")->result();
			...
			if (is_array($stok)) :
				foreach ($stok as $i => $val) :
					...
					$data_stok = array(
						'waktu'        => $this->datetime,
						'id_transaksi' => (int)$id_distribusi,
						'transaksi'    => 'Distribusi',
						'id_barang'    => (int)$val->id_barang,
						'ed'           => isset($val->ed) ? $val->ed : NULL,
						'keluar'       => $keluar,
						'keterangan'   => $ket_unit,
						'id_unit'      => (int)$this->unit,
						'id_users'     => (int)$this->user,
						'id_stok'      => (int)$val->id_stok
					);
