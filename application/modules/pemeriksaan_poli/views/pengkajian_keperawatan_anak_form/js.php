...
<script>
    ...
    // PAKARJ
    function entriFormPengkajianAwalKeperawatanAnakRawatJalan(details, id_pasien, data_page, layanan, tanggal, id_pendaftaran) {
        // $('#modal_form_keperawatan_anak').modal('show');  
              
        let detail = details.split('#');
        $('#id-layanan-pendaftaran-pakarj').val(detail[0]);
        $('#id-pendaftaran-pakarj').val(id_pendaftaran);
        // console.log(detail);
        $.ajax({
            type: 'GET',
            url: '<?= base_url('pemeriksaan_poli/api/pemeriksaan_poli/get_data_pengkajian_keperawatan_anak') ?>/id/' +id_pendaftaran +'/id/' + detail[0],
            cache: false,
            dataType: 'JSON',
            beforeSend: function() {
                showLoader();
            ...
            success: function (data) {
                ...
                if (data.pasien !== null) {
                    // $('#id-keperawatan-anak').val(data.pasien.id_pasien);
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                if (data.respon !== null) {
                    ...
                    if (data.status) {
                        messageAddSuccess();
                        $('#modal_form_keperawatan_anak').modal('hide');
