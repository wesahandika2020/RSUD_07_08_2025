			...
			<table class="small__font">
				...
				<tbody class="border">
					...
					<tr>
						<td class="no__border grid__signature">
							<div></div>
							<div>
								Tangerang, <?= tanggal_indo($surat_pengantar_rawat->tanggal_dokter_spr, false) ?></b><br>
								Dokter Pengirim,<br>

								<?php if (!empty($surat_pengantar_rawat->ttd_dokter_2)) :
									echo '<img src="' . resource_url() . 'images/ttd_dokter/' . $surat_pengantar_rawat->ttd_dokter_2 . '" alt="ttd-dokter" width="200"><br>';
								else :
									echo '<br><br><br><br><br>';
								endif;
								?>

								( <?= $surat_pengantar_rawat->nama_dokter_2; ?>)<br>
								Nama & Tanda Tangan
							</div>
						</td>
					</tr>
				...
				<tbody class="border">
					<tr>
						<td class="no__border grid__two-column">
							...
							<div>
								<?php
								// Ambil isi catatan
								$catatan = $surat_pengantar_rawat->catatan_pendaftaran_spr;

								// Ubah newline (untuk Windows, Unix, Mac) menjadi <br> agar enter muncul di halaman web
								$catatan_dengan_newline = preg_replace('/\r\n|\r|\n/', '<br>', htmlspecialchars($catatan));

								// Ubah pola angka diikuti titik menjadi tag <br> untuk memisahkan catatan berdasarkan angka
								$catatan_dengan_format = preg_replace('/(\d+\.\s*)/', '<br>$1', $catatan_dengan_newline);

								// Tampilkan hasilnya
								echo $catatan_dengan_format;
								?>
							</div>
						</td>
					</tr>
					...
					<!-- <td class="no__border grid__signature">
						<div></div>
						<div>
							Tangerang, <!?= tanggal_indo($surat_pengantar_rawat->tanggal_petugas_spr, false) ?></b><br>
							Petugas Pendaftaran,<br>

							<!?php if (!empty($surat_pengantar_rawat->ttd_users)) :
								echo '<img src="' . resource_url() . 'images/ttd_dokter/' . $surat_pengantar_rawat->ttd_users . '" alt="ttd-dokter" width="200"><br>';
							else :
								echo '<br><br><br><br><br>';
							endif;
							?>

							( <!?= $surat_pengantar_rawat->id_users; ?>)<br>
							Nama & Tanda Tangan
						</div>
					</td> -->
					...
					<td class="no__border grid__signature">
						...
						<div>
							...
							<?php if (!empty($surat_pengantar_rawat->ttd_petugas)) :
								echo '<img src="' . resource_url() . 'images/ttd_dokter/' . $surat_pengantar_rawat->ttd_petugas . '" alt="ttd-dokter" width="200"><br>';
							else :
								echo '<br><br><br><br><br>';
							endif;
							?>
							( <?= $surat_pengantar_rawat->nama_petugas_pendaftaran; ?>)<br>
							Nama & Tanda Tangan
						</div>
					</td>
				</tbody>
