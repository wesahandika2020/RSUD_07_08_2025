...
<script>
  $(document).ready(function() {
    ...
    var idPendaftaran = '<?= $eclaim->id_pendaftaran ?>';
    var jenis_rawat = '<?= $pendaftaran->jenis_rawat ?>';

    var eclaim = <?= json_encode($eclaim) ?>;
    var status_eklaim = <?= json_encode($status_eklaim) ?>;
    var pasien = <?= json_encode($pendaftaran) ?>;
    var kelahiran = <?= json_encode($data_kelahiran) ?>;
    var status_grouper = <?= json_encode($status_grouper) ?>;

    // if (onClick) {
    loadAllEclaimICD(idPendaftaran, eclaim.status_klaim, eclaim.method_status);
    // }
    ...
    $('#id-pendaftaran-gpr').val(eclaim.id_pendaftaran);
    $('#id-layanan-pendaftaran-gpr').val(eclaim.id);
    ...
    if (eclaim.status_klaim == null) {
      ...
      let TarifRad = (eclaim.tarif_radiologi !== null ? parseInt(eclaim.tarif_radiologi) : 0);
      let TarifLab = (eclaim.tarif_laboratorium !== null ? parseInt(eclaim.tarif_laboratorium) : 0);
      let TarifKamar = (eclaim.tarif_kamar !== null ? parseInt(eclaim.tarif_kamar) : 0);
      let TarifAkomodasi = (eclaim.tarif_akomodasi !== null ? parseInt(eclaim.tarif_akomodasi) : 0);
      let TarifICare = (eclaim.tarif_intensive_care !== null ? parseInt(eclaim.tarif_intensive_care) : 0);
      let TarifDarah = (eclaim.tarif_pelayanan_darah !== null ? parseInt(eclaim.tarif_pelayanan_darah) : 0);
      let TarifHemo = (eclaim.tarif_hemodialisa !== null ? parseInt(eclaim.tarif_hemodialisa) : 0);
      let TarifKonsul = (eclaim.tarif_konsultasi !== null ? parseInt(eclaim.tarif_konsultasi) : 0);
      let TarifKeperawatan = (eclaim.tarif_keperawatan !== null ? parseInt(eclaim.tarif_keperawatan) : 0);
      let TarifTenagaAhli = (eclaim.tarif_tenaga_ahli !== null ? parseInt(eclaim.tarif_tenaga_ahli) : 0);
      let TarifRehab = (eclaim.tarif_rehabilitas !== null ? parseInt(eclaim.tarif_rehabilitas) : 0);
      let TarifNonBedah = (eclaim.tarif_non_bedah_vk !== null ? parseInt(eclaim.tarif_non_bedah_vk) : 0);
      let TarifBedah = (eclaim.tarif_bedah_ok !== null ? parseInt(eclaim.tarif_bedah_ok) : 0);
      let TarifObat = (eclaim.tarif_obat !== null ? parseInt(eclaim.tarif_obat) : 0);
      let TarifBHP = (eclaim.tarif_bhp !== null ? parseInt(eclaim.tarif_bhp) : 0);
      let TarifKronis = (eclaim.tarif_obat_kronis !== null ? parseInt(eclaim.tarif_obat_kronis) : 0);
      let TarifKemoterapi = (eclaim.tarif_obat_kemoterapi !== null ? parseInt(eclaim.tarif_obat_kemoterapi) : 0);
      let TarifAlkes = (eclaim.tarif_alkes !== null ? parseInt(eclaim.tarif_alkes) : 0);
      let TarifSewaAlat = (eclaim.tarif_sewa_alat !== null ? parseInt(eclaim.tarif_sewa_alat) : 0);
      ...
      <?php
      // Ambil dan gabungkan kode dari diagnosa
      $diagnosis_val = '';
      if (!empty($diagnosa)) {
        $diagnosis_codes = array_map(function ($diag) {
          return $diag->code;
        }, $diagnosa);
        $diagnosis_val = implode('#', $diagnosis_codes);
      }

      // Ambil dan gabungkan kode dari procedure (dan tambahan lain jika ada)
      $procedure_val = '';
      if (!empty($procedure)) {
        $procedure_codes = array_map(function ($proc) {
          return $proc->code;
        }, $procedure);
        $procedure_val = implode('#', $procedure_codes);
      }
      ?>

      $('#gpr_procedure').val('<?= $procedure_val ?>');
      $('#gpr_diagnosis').val('<?= $diagnosis_val ?>');
      ...
      if (eclaim.method_status == 'grouper' && status_grouper !== null) {
  ...
  function closeTabEklaim() {
    // $('#tab-index-eklaim').addClass('tab-content');
    loadMenuReload('eklaim_bpjs/Eklaim_bpjs/page_coding_grouping/<?= $status_eklaim->nomor_rm ?>', 'Eklaim v.5.9.x', '', 'Modal Eklaim')
  }


  function loadAllEclaimICD(idPendaftaran, statusKlaim, methodStatus) {
    ...
    $.ajax({
      type: 'GET',
      url: '<?= base_url("eklaim_bpjs/api/Eklaim_bpjs/get_all_eklaim_data") ?>',
      data: {
        id: idPendaftaran,
      ...
      success: function(data) {
        renderDiagnosaGeneric('unu', data.diagnosa_unu, idPendaftaran, statusKlaim);
        renderDiagnosaGeneric('ina', data.diagnosa_ina, idPendaftaran, statusKlaim);
        renderProsedurGeneric('unu', data.prosedur_unu, idPendaftaran, statusKlaim);
        renderProsedurGeneric('ina', data.prosedur_ina, idPendaftaran, statusKlaim);
      },
      complete: hideLoader,
      error: function() {
        messageCustom('Gagal memuat data e-klaim', 'Error');
      }
    });
  }

  // Diagnosa
  function renderDiagnosaGeneric(tipe, data, id_pendaftaran, status_klaim) {
    const apgarCodes = ["73.8", "73.92", "74.4"];
    const HDCodes = ["39.95"]; // Kode diagnosa HD
    let html = '';
    let apgarVisible = false;

    if (data.length !== 0) {
      $.each(data, function(i, v) {
        let prioritas = i === 0 ? 'Primer' : 'Sekunder';
        let btnSetPrimer = (i === 0) ? '' : `
        <button type="button" onclick="setPrimerDiagnosa('${tipe}', '${id_pendaftaran}', '${v.code}')" class="btn btn-secondary">
          <i class="far fa-edit mr-1"></i>Set Primer
        </button>`;

        if (apgarCodes.includes(v.code)) apgarVisible = true;

        if (status_klaim === 'Terkirim' || status_klaim === 'Final') {
          html += `
          <tr>
            <td style="border-bottom: 1px solid #aaa; padding: 5px;">
              <div class="row ml-1">
                ${tipe === 'ina' ? '<h6><span class="fas fa-list-ul mr-2"></span></h6>' : ''}
                <h6>${v.nama}<span class="badge badge-primary ml-1">${v.code}</span></h6>
                <span class="ml-1">${prioritas}</span>
              </div>
            </td>
          </tr>`;
        } else {
          html += `
          <tr class="diagnosis_${tipe}">
            <td style="border-bottom: 1px solid #aaa; padding: 5px;">
              <div class="row ml-1" id="label-edit-diag-${tipe}${i+1}">
                ${tipe === 'ina' ? '<h6><span class="fas fa-list-ul mr-2"></span></h6>' : ''}
                <h6 class="pointer" onclick="toggleEditDiag('${tipe}${i+1}')">${v.nama}<span class="badge badge-primary ml-1">${v.code}</span></h6>
                <span class="ml-1">${prioritas}</span>
              </div>
              <div class="row" id="edit-diag-${tipe}${i+1}" style="display: none;">
                <div class="col-lg-3">
                  <input type="text" class="form-control" style="border-radius: 25px;" onkeydown="handleEnterDiagNEW(event, '${tipe}${i+1}')" id="search-diagnosa-${tipe}${i+1}" placeholder="Cari Diagnosa">
                  <div id="diagnosaResult-${tipe}${i+1}" class="diagnosa-${tipe}${i+1} diagnosa-class" style="display: none;"></div>
                </div>
                <div class="col-lg-1">
                  <button type="button" onclick="hapusDiagnosaNEW(this, '${tipe}', '${id_pendaftaran}', '${v.code}')" class="btn btn-danger">
      ...
      $(`#table-list-diagnosa-${tipe} tbody`).empty().append(html);
      ...
      // Combine codes
      // const combinedCodes = data.map(item => item.code).join('#');
  ...
  // Procedure
  function renderProsedurGeneric(tipe, data, id_pendaftaran, status_klaim) {
    const apgarCodes = ["73.8", "73.92", "74.4"];
    const HDCodes = ["39.95"]; // Kode diagnosa HD
    let html = '';
    let apgarVisible = false;
    ...
    if (data.length !== 0) {
      $.each(data, function(i, v) {
        let prioritas = i === 0 ? 'Primer' : 'Sekunder';
        let btnSetPrimer = (i === 0) ? '' : `
        <button type="button" onclick="setPrimerProsedur('${tipe}', '${id_pendaftaran}', '${v.code}')" class="btn btn-secondary">
          <i class="far fa-edit mr-1"></i>Set Primer
        </button>`;
        ...
        <tr>
          ...
          html += `
        <tr class="procedure_${tipe}">
          <td style="border-bottom: 1px solid #aaa; padding: 5px;">
            <div class="row ml-1" id="label-edit-proc-${tipe}${i+1}">
              ${tipe === 'ina' ? '<h6><span class="fas fa-list-ul mr-2"></span></h6>' : ''}
              <h6 class="pointer" onclick="toggleEditProc('${tipe}${i+1}')">${v.nama}<span class="badge badge-primary ml-1">${v.code}</span></h6>
              <span class="ml-1">${prioritas}</span>
            </div>
            <div class="row" id="edit-proc-${tipe}${i+1}" style="display: none;">
              <div class="col-lg-3">
                <input type="text" class="form-control" style="border-radius: 25px;" onkeydown="handleEnterProcNEW(event, '${tipe}${i+1}')" id="search-prosedur-${tipe}${i+1}" placeholder="Cari Prosedur">
                <div id="prosedurResult-${tipe}${i+1}" class="prosedur-${tipe}${i+1} prosedur-class" style="display: none;"></div>
              </div>
              <div class="col-lg-1">
                <button type="button" onclick="hapusProsedurNEW(this, '${tipe}', '${id_pendaftaran}', '${v.code}')" class="btn btn-danger">
                  <i class="fas fa-trash-alt mr-1"></i>Hapus
                </button>
              </div>
              <div class="col-lg-2">${btnSetPrimer}</div>
      ...
      $(`#table-list-prosedur-${tipe} tbody`).empty().append(html);
    ...
    $.ajax({
      ...
      success: function(data) {
        ...
        if (data.status) {
          ...
          loadMenuReload('eklaim_bpjs/Eklaim_bpjs/modal_eklaim/<?= $status_eklaim->id_pendaftaran ?>/<?= $pendaftaran->jenis_rawat ?>', 'E-Klaim', '', 'Modal Eklaim')
        } else {
  ...
  function showGrouperResult(jenis_rawat) {
    let grouper = <?= json_encode($status_grouper) ?>;
    let eclaim = <?= json_encode($eclaim) ?>;
    let pasien = <?= json_encode($pendaftaran) ?>;
    let status_eklaim = <?= json_encode($status_eklaim) ?>;
