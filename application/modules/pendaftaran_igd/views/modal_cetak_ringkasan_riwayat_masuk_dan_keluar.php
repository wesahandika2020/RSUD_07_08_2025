<script>
	...
	function simpanRingkasanRiwayatMasukDanKeluar() {
		...
		$.ajax({
			type: 'POST',
			url: '<?= base_url( "pendaftaran_igd/api/pendaftaran_igd/simpan_ringkasan_riwayat_masuk_dan_keluar" ) ?>',
			...
			success: function (data) {
				if (data.status) {
					...
					window.open('<?= base_url( 'pendaftaran_igd/cetak_ringkasan_riwayat_masuk_dan_keluar/' ) ?>' + id, 'Cetak Ringkasan Riwayat Masuk dan Keluar', 'width=' + dWidth + ', height=' + dHeight + ', left=' + x + ', top=' + y);
				} else {
