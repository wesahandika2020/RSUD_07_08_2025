<div id="modal-bed-icare" class="modal fade">
	<div class="modal-dialog modal-dialog-scrollable" style="max-width: 90%;">
		<div class="modal-content">
						...
						<div class="row">
								...
								<table class="table	table-striped table-hover table-sm">
									<tbody>
										<tr>
											...
											<td width="1%">:</td>
											<td width="69%"><span class="nama-icare-label"></span></td>
										</tr>
										<tr>
											<td><b>No. RM</b></td>
											<td>:</td>
											<td><span class="no-rm-icare-label"></span></td>
										</tr>
										<tr>
											<td><b>Kelamin</b></td>
											<td>:</td>
											<td><span class="kelamin-icare-label"></span></td>
										</tr>
									</tbody>
								</table>
							</div>
							<div class="col-lg-6">
								<table class="table	table-striped table-hover table-sm">
									<tbody>
										<tr>
											<td width="30%"><b>Layanan Asal</b></td>
											<td width="1%">:</td>
											<td width="69%"><span class="layanan-icare-label"></span></td>
										</tr>
										<tr>
											<td><b>Cara Bayar</b></td>
											<td>:</td>
											<td><span class="cara-bayar-icare-label"></span></td>
										</tr>
										<tr>
											<td><b>Hak Kelas Pasien</b></td>
											<td>:</td>
											<td><span class="hak-kelas-pasien"></span></td>
										</tr>
										<!--<tr>
											<td><b>Bangsal Tujuan</b></td>
											<td>:</td>
											<td><span class="bangsal-tujuan-icare-label"></span></td>-->
										</tr>
									</tbody>
								</table>
							</div>
							...
							<div class="col-lg-3">
								<div class="bs-docs-sidebar hidden-print" role="complementary">
									...
									<div id="bangsalScroll">
										<ul class="nav bs-docs-sidenav list_bangsal">
			...
			<div class="modal-footer">
				<button type="button" class="btn btn-info" onclick="closeModalBedIcare()"><i class="fas fa-check-circle mr-1"></i>OK</button>
			</div>
		</div>
	</div>
</div>
...
<script type="text/javascript">
	...
	function getAvailableBed(id_bangsal, tipe) {
		$.ajax({
			type: 'GET',
			url: '<?= base_url("order_intensive_care/api/order_intensive_care/get_available_bed") ?>/id_bangsal/' + id_bangsal,
			cache: false,
			dataType: 'JSON',
			beforeSend: function() {
				showLoader()
						...
						'<div class="card-header">'+
							'<div class="d-flex flex-row">'+
								...
								'<div class="align-self-center m-l-10">'+
									'<h4 class="m-b-0 text-info"><b>Kelas '+i+'</b></h4>'+
								'</div>'+
							'</div>'+
						'</div>'+
						'<div class="card-body">';
	...
	function showListBedEdit(data) {
		...
		$.each(data.kelas, function(i, value) {
			...
			$.each(value, function(j, value2) {
				html += '<div class="row"><div class="col-lg-12">';
				html += '<h4 class="center"><b>Ruang '+j+'</b></h4><div class="col-md-12 d-flex justify-content-center mb-2">';
				...
				$.each(value2, function(k, value3) {
					...
					let theBed = value3.bangsal + ', kelas ' + value3.kelas + ', kamar ' + value3.no_ruang + ', nomor bed ' + value3.no_bed;
					action = 'onclick="setBedEdit('+value3.id+', \''+theBed+'\')"';
					...
					} else if (value3.keterangan === 'Siap Dipakai') {
						action = '';
						pointer = '';
					}
	...
	function getListBangsal(id_bangsal_tujuan = null) {
		$('ul.list_bangsal').empty()
		if (id_bangsal_tujuan !== null) {
			$.ajax({
				type: 'GET',
				url: '<?php echo base_url('order_rawat_inap/get_list_bangsal') ?>',
				data: 'id_bangsal=' + id_bangsal_tujuan,
				cache: false,
				dataType: 'JSON',
				success: function(data) {
					if (data) {
						$.each(data, function(i, v) {
							$('ul.list_bangsal').append(`
								<li class="li_side pointer" onclick="getAvailableBed(${v.id})">
									<a style="font-size: 14pt">${v.nama}</a>
								</li>
							`);
						})
					}
