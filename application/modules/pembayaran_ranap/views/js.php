<script>
	...
	function getListPembayaran(page) {
		...
		$.ajax({
			type: 'GET',
			url: '<?= base_url("keuangan/api/keuangan/get_list_rekap_billing") ?>/page/' + page + '/jenis/rawat_inap',
				...
				$.each(data.data, function(i, v) {
					...
					if (v.status_billing === 'Batal') {
						...
						if (v.lunas === 'Belum') {
							...
							if (parseInt(v.diskon_asuransi) === 100) {
								...
								status = '<em class="blinker"><i class="fas fa-spinner fa-spin mr-1"></i>Belum Lunas</em>';
							}
						} else {
					...
					let html = /* html */ `
						<tr>
							...
							<td class="center wrapper">${status}</td>
							<td class="right aksi">
								${buttonBayar}
