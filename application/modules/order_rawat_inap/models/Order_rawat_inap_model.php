...
class Order_rawat_inap_model extends CI_Model {
	...
	private function sqlOrderRawatInap($search)
    {
            ...
            $this->db->from('sm_order_rawat_inap as odr')
				...
				->join('sm_rawat_inap as ri2', 'ri2.id = odr.id_rawat_inap', 'left');
			...
			$idAccountGroup = $this->session->userdata('id_account_group');
				   if ($idAccountGroup == '130') { $paramBangsal = '5'; 	 // Karu Jati
			} else if ($idAccountGroup == '131') { $paramBangsal = '3,4'; 	 // Karu Cendana
			} else if ($idAccountGroup == '132') { $paramBangsal = '16,17';  // Karu Ulin
			} else if ($idAccountGroup == '133') { $paramBangsal = '18,6,8'; // Karu Eboni dan Alba
			} else { $paramBangsal = null; }

			if ($search['unit'] !== '' & $paramBangsal !== null) :
                $this->db->where('bgt.id in ('. $paramBangsal .')');
            endif;

			if ($paramBangsal == null & $this->session->userdata('account_group') !== 'Admin' & $this->session->userdata('account_group') !== 'Rawat Inap' & $this->session->userdata('account_group') !== 'Supervisor Medical Record') :
    ...
    private function _listOrderRawatInap($limit = 0, $start = 0, $search)
    {
        $this->sqlOrderRawatInap($search);
		if ($limit !== 0) :
    ...
    function getListDataOrderRawatInap($limit = 0, $start = 0, $search)
    {
        $result['data'] = $this->_listOrderRawatInap($limit, $start, $search);
        $result['jumlah'] = $this->sqlOrderRawatInap($search)->get()->num_rows();
        return $result;

        $this->db->close();
	}
	
	function updatePembatalanOrderRawatInap($id_order, $keterangan)
	{
		$this->db->trans_begin();
		$sql = "select odr.*, lp.jenis_layanan 
				from sm_order_rawat_inap as odr 
		...
		$this->db->where('id', $id_order)->update('sm_order_rawat_inap', $data);
		...
		if ($this->db->trans_status() === false) :
			...
			$status = false;
			$message = 'Gagal melakukan pembatalan order rawat inap';
		else :
			$this->db->trans_commit();
			$status = true;
			$message = 'Berhasil melakukan pembatalan order rawat inap';
		endif;

		$this->load->model('logs/Logs_model', 'm_logs');
		$this->m_logs->recordAdminLogs($dataOrder->id_layanan_pendaftaran, 'Batal Order Rawat Inap');
	...
	function getDataDetailOrderRawatInap($id_order)
	{
	...
	function getDataAvailableBed($id_bangsal, $keterangan)
	{
		...
		$kelas = array("Non Kelas" => NULL, "I" => NULL, "II" => NULL, "III" => NULL, "VIP" => NULL);
		...
		$selectBedSQL = "select
						CONCAT_WS(' ' , ru.no_ruang, ru.keterangan) as no_ruang, 
						ru.id_kelas, 
						ru.keterangan as keterangan_ruang,
						k.nama as kelas, 
		...
		$sql = "from sm_bed as b
				...
				and b.is_active = '1' ";
		// run SQL
		// echo $selectBangsalSQL.$sql.$query.$orderBangsalSQL; die;
		$dataBangsal = $this->db->query($selectBangsalSQL.$sql.$query.$orderBangsalSQL)->row();
		if (0 < count((array)$dataBangsal)) :
			$queryBedSQL = " and bg.id = '".$dataBangsal->id."' order by ru.no_ruang, b.no_bed";
			...
			foreach ($dataBangsal->kelas as $kelas => $valueKelas) :
				...
				if (0 < count((array)$ruang)) :
					...
					endforeach;
					$dataBangsal->kelas[$kelas] = $ruangKelas;
				else :
	...
	// POST Rawat Inap
	function simpanDataRawatInap($data)
	{
		$this->db->insert('sm_rawat_inap', $data);
	...
	function setOrderRawatInap($id_order, $id_rawat_inap)
	{
		$data = array('status' => 'dirawat', 'id_rawat_inap' => $id_rawat_inap);
		$this->db->where('id', $id_order, true)->update('sm_order_rawat_inap', $data);
	}
