...
<script>
    $(function() {
        ...
        $('#btn-tambah').click(function() {
            $('#modal-barang-farmasi').modal('show');
            $('#modal-barang-farmasi-label').html('Tambah Barang');
            resetForm();
            $('.deletebarang').removeAttr('disabled');
            $('.packing').html('');
            $('#table-kandungan tbody').empty();
        ...
        $('#btn-excel').click(function() {
            location.href='<?= base_url('barang_farmasi/export_data_barang') ?>?'+$('#form-search').serialize()+'&keyword='+$('#pencarian').val();
        });
        ...
        $('#kls-terapi').select2({
            ajax: {
                url: "<?= base_url('masterdata/api/masterdata_auto/farmako_terapi_auto/') ?>",
                dataType: 'json',
                quietMillis: 100,
                data: function (term, page) { // page is the one-based page number tracked by Select2
                    return {
        ...
        $('#kategori-barang').change(function() {
            // id 26 adalah kategori bahan baku
            if ($(this).val() === '26') {
                $('.bahan_baku').show()
                $('#bahan_baku').focus()
            } else {
    ...
    function hitung_hja() {
        var hpp         = parseFloat(currencyToNumber($('#hpp').val()));
        var margin_r    = $('#margin-resep-pr').val();
        var margin_nr   = $('#margin-non-resep-pr').val();
        var margin_rp_r = hpp*(margin_r/100);
        var margin_rp_nr= hpp*(margin_nr/100);
        $('#margin-resep-rp').val(money_format(margin_rp_r));
        $('#margin-non-resep-rp').val(money_format(margin_rp_nr));
        hitung_hpp();
    }

    function hitung_hpp() {
        var harga_dasar         = parseFloat(currencyToNumber($('#harga_dasar').val()));
        // ppn 11%
        var hpp         = harga_dasar+(harga_dasar*0.11);
        $('#hpp').val(money_format(hpp));
    }

    function hitung_harga_dasar() {
        var hpp         = parseFloat(currencyToNumber($('#hpp').val()));
        var harga_dasar         = hpp/1.1;
        $('#harga_dasar').val(money_format(harga_dasar));
        var margin_r    = $('#margin-resep-pr').val();
        var margin_nr   = $('#margin-non-resep-pr').val();
        var margin_rp_r = hpp*(margin_r/100);
        var margin_rp_nr= hpp*(margin_nr/100);
            ...
            buttons: {
                ...
                hapus: {
                    ...
                    callback: function() {
                        ...
                        $.ajax({
                            type : 'DELETE',
                            url: '<?= base_url("barang_farmasi/api/barang_farmasi/kemasan_barang") ?>/id/'+id,
                            cache: false,
                            dataType : 'JSON',
    ...
    function penyesuaianAtribut() {
        var jml = $('.mother').length;
        for (i = 0; i <= (jml-1); i++) {
            $('.mother:eq('+i+')').attr('id','mother'+i);
            $('.mother:eq('+i+')').children('td:eq(0)').children('.id-kemasan').attr('name','id_kemasan'+i);
            $('.mother:eq('+i+')').children('td:eq(0)').children('.id-kemasan').attr('id','id-kemasan'+i);

            $('.mother:eq('+i+')').children('td:eq(0)').children('.barcode').attr('name','barcode'+i);
            $('.mother:eq('+i+')').children('td:eq(0)').children('.barcode').attr('id','barcode'+i);

            $('.mother:eq('+i+')').children('td:eq(0)').children('.kemasan').attr('name','kemasan'+i);
            $('.mother:eq('+i+')').children('td:eq(0)').children('.kemasan').attr('id','kemasan'+i);
            $('.mother:eq('+i+')').children('td:eq(0)').children('.kemasan').attr('onchange','isiSatuanTerkecil('+i+')');

            $('.mother:eq('+i+')').children('td:eq(0)').children('.isi').attr('name','isi'+i);
            $('.mother:eq('+i+')').children('td:eq(0)').children('.isi').attr('id','isi'+i);

            $('.mother:eq('+i+')').children('td:eq(0)').children('.satuan').attr('name','satuan'+i);
            $('.mother:eq('+i+')').children('td:eq(0)').children('.satuan').attr('id','satuan'+i);
            $('.mother:eq('+i+')').children('td:eq(0)').children('.satuan').attr('onchange','configAutoSuggest('+i+')');

            $('.mother:eq('+i+')').children('td:eq(0)').children('.isi-kecil').attr('name','isi_kecil'+i);
            $('.mother:eq('+i+')').children('td:eq(0)').children('.isi-kecil').attr('id','isi-kecil'+i);

            $('.mother:eq('+i+')').children('td:eq(0)').children('button').attr('onclick','deleteSettingHarga('+i+')');
            $('.mother:eq('+i+')').children('td:eq(0)').children('.jumlah').attr('value',i);
        }
    }

    function addNewKandungan(id, nama) {
        let html = /* html */ `
            <tr class="kandungan" style="height:37px">
                <td>
                    <input type="hidden" name="id_kandungan[]" value="${id}">
                    <input type="text" name="kandungans[]" class="form-control" value="${nama}">
                </td>
                <td>
                    <button type="button" class="btn btn-secondary btn-lg" onclick="removeKandungan(this)"><i class="fas fa-trash mr-2"></i>Hapus</button>
                </td>
            </tr>
        `;

        $('#table-kandungan tbody').append(html);
    }

    function removeKandungan(el) {
    ...
    function autoAdjustment() {
        ...
        for (i = 0; i <= (jml-1); i++) {
            if (jml > 1) {
                var isi = $('#isi'+(i+1)).val();
                $('#isi-kecil'+i).val(((isi !== undefined)?isi:1));
            }
        }
        penyesuaianAtribut();
    }
    ...
    function konfirmasiSimpanData() {
        ...
        if (jmlPacking > 3) {
            ...
            $('#wizard-form').bwizard('show', '2'); return false;
        }
        ...
        if ($('#stok-min').val() === '') {
            syams_validation('#stok-min', 'Kolom stock minimal harus diisi minimal 0.');
            $('#stok-min').focus();
        ...
        if ($('#harga_dasar').val() === '') {
            syams_validation('#harga_dasar', 'Kolom harga dasar harus diisi.');
            $('#harga_dasar').focus();
            $('#wizard-form').bwizard('show', '1');
            return false;
        }
        ...
        if ($('#hna').val() === '') {
            syams_validation('#hna', 'Kolom harga netto apotik harus diisi.');
            $('#hna').focus();
            $('#wizard-form').bwizard('show', '1');
            return false;
        }
        ...
        if ($('#hpp').val() === '') {
            syams_validation('#hpp', 'Kolom harga penjualan pokok harus diisi.');
            $('#hpp').focus();
            $('#wizard-form').bwizard('show', '1');
            return false;
        }

        if ($('#margin-resep-pr').val() === '') {
            syams_validation('#margin-resep-pr', 'Kolom margin resep persen harus diisi.');
            $('#margin-resep-pr').focus();
            $('#wizard-form').bwizard('show', '1');
            return false;
        }
        
        if ($('#margin-non-resep-pr').val() === '') {
            syams_validation('#margin-non-resep-pr', 'Kolom margin non resep persen harus diisi.');
            $('#margin-non-resep-pr').focus();
            $('#wizard-form').bwizard('show', '1');
            return false;
        }
        ...
        if ($("#kategori-barang").val() === '1' && $('#kekuatan').val() === '') {
            syams_validation('#kekuatan', 'Kolom kekuatan harus diisi, jika barang tidak termasuk obat bisa diisikan 0');
            $('#wizard-form').bwizard('show', '0'); return false;
        }
        syams_validation_remove('#kekuatan');

        if ($('#kategori-barang').val() === '1' && $('#satuan-kekuatan').val() === '') {
            syams_validation('#satuan-kekuatan', 'Kolom satuan kekuatan harus diisi.');
            return false;
        }
        syams_validation_remove('#satuan-kekuatan');
    ...
    function simpanData() {
        ...
        $.ajax({
            type: 'POST',
            url: '<?= base_url("barang/api/barang/simpan_barang") ?>' + update,
            data: $('#form-barang-farmasi').serialize(),
            cache: false,
            dataType: 'JSON',
            beforeSend: function() {
                showLoader();
            ...
            success: function(data) {
                ...
                $('#modal-barang-farmasi').modal('hide');
            },
            complete: function() {
                hideLoader();
            },
    ...
    function getListBarang(p, id_barang) {
        ...
        $.ajax({
            type: 'GET',
            url: '<?= base_url("barang_farmasi/api/barang_farmasi/get_list_barang_farmasi") ?>/page/' + p + '/id' + id,
            ...
            success: function(data) {
                ...
                $.each(data.data, function(i, v) {
                    if (v.generik === 'Tidak') { gen = '&cross;'; }
                    if (v.generik === 'Ya') { gen = '&checkmark;'; }
                    if (v.generik === null) { gen = '-'; }

                    if (v.formularium === 'Tidak') { form = '&cross;'; }
                    if (v.formularium === 'Ya') { form = '&checkmark;'; }
                    if (v.formularium === null) { form = '-'; }
                    ...
                    let html = /* html */ `
                        <tr ${redBlock}>
                            ...
                            <td>${v.kekuatan}&nbsp;${v.satuan}</td>
                            <td>${v.sediaan}</td>
                            <td><small><i>${v.pabrik}</i></small></td>
                            <td class="right">${money_format(v.harga_dasar)}</td>
                            ...
                            <td class="center">${form}</td>
                            <td class="center">${v.ekatalog}</td>
                            <td class="center">${((v.high_alert === 'Tidak') ? '&cross;' : '&checkmark;')}</td>
                            <td class="center" style="white-space: nowrap;">${v.kategori}</td>
                            <td class="center">${((v.is_obat_kronis === '1') ? '&checkmark;' : '&cross;')}</td>
    ...
    function editBarang(id) {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                $('#sediaan').val(data.id_sediaan);
                $('#satuan-kekuatan').val(data.satuan_kekuatan);
                $('#roa').val(data.roa);
                $('#asal').val(data.status);
                $('#katastropik').val(data.katastropik);
                $('#generik').val(data.generik);
                $('#golongan').val(data.id_golongan);
                $('#fno, #fyes, #katno, #katyes, #fnno, #fnyes, #ekatalogyes, #ekatalogno, #hno, #hyes').removeAttr('checked');
                $('#formularium').val(data.formularium);
                $('#fornas').val(data.fornas);
                if (data.ekatalog === 'Ya') {
                    $('#ekatalogyes').attr('checked','checked');
                }
                if (data.ekatalog === 'Tidak') {
                    $('#ekatalogno').attr('checked','checked');
                }
                $('#obatkronisyes').prop('checked',data.is_obat_kronis === '1');
				$('#obatkronisno').prop('checked',data.is_obat_kronis === '0');
                $('#highalert').val(data.high_alert);
                $('#stok-min').val(data.stok_minimal);
                $('#rak').val(data.rak);
                $('#harga_dasar').val(money_format(data.harga_dasar));
                $('#hna').val(money_format(data.hna));
                $('#hpp').val(money_format(data.hpp));
                $('#margin-non-resep-pr').val(data.margin_non_resep);
                $('#margin-resep-pr').val(data.margin_resep);
                $('#farmakoterapi').val(data.id_farmakoterapi);

                $('#indikasi').val(data.indikasi);
                $('#dosis').val(data.dosis);
                $('#kandungan').val(data.kandungan);
                $('#perhatian').val(data.perhatian);
                $('#kontra-indikasi').val(data.kontra_indikasi);
                $('#efek-samping').val(data.efek_samping);
                $('#kategori-barang').val(data.id_kategori);
                
                if (data.id_kategori === '26') {
                    $('.bahan_baku').show()
                ...
                $('#kls-terapi').val(data.id_farmako_terapi);
                $('#s2id_kls-terapi a .select2-chosen').html(data.farmakoterapi);
                $('#atdps').val(data.atdps);
                $('#antibiotik').val(data.antibiotik);
                $('#obatkemo').val((data.kemo === '1')?'Ya':'Tidak');

                let hpp         = parseFloat(currencyToNumber($('#hpp').val()));
                let margin_r    = $('#margin-resep-pr').val();
                let margin_nr   = $('#margin-non-resep-pr').val();
                let margin_rp_r = hpp*(margin_r/100);
                ...
                $('#table-kandungan tbody').empty();
                $.each(data.kandungans, function(j, values) {
                    addNewKandungan(values.id, values.nama_kandungan);
                });

                $('#modal-barang-farmasi').modal('show');
                $('#modal-barang-farmasi-label').html('Edit Barang');
            },
            error: function(e) {
                accessFailed(e.status);
            }
