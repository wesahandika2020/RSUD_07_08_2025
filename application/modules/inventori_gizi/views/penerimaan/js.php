<script>
	$(function () {
		...
		$('#add').click(function() {
            ...
            if (pembayaran === null ) {
                syams_validation('#pembayaran', 'Kategori Pembayaran tidak boleh kosong !'); return false;
            }

            addNewRows(id_barang, nama_barang, jumlah, id_kemasan, pembayaran, keterangan)
		})
	...
	function addNewRows(id_barang, nama_barang, jumlah, id_kemasan, pembayaran, keterangan) {
		...
		let html = /* html */ `
			<tr class="rows">
				...
				<td class="center">${pembayaran}<input type="hidden" name="pembayaran[]" id="pembayaran${i}" value="${pembayaran}" class="pembayaran"></td>
				<td class="left">${keterangan}<input type="hidden" name="keterangan[]" id="keterangan${i}" value="${keterangan}" class="keterangan"></td>
		...
		$.ajax({
            url: '<?= base_url('inventori_gizi/api/inventori_gizi/get_detail_harga_barang_pemesanan') ?>?id=' + id_barang + '&id_kemasan=' + id_kemasan,
            dataType: 'JSON',
            cache: false,
	...
	function hitungDiskon(i){

		let disc = 0;

		if($('#jml'+i).val() !== '' && $('#hna'+i).val() !== ''){
			var jumlah = parseInt($('#jml'+i).val());
			var hna = currencyToNumber($('#hna'+i).val());
			var discRp = currencyToNumber($('#disc-rp'+i).val());
				
			disc = hna*jumlah;
			disc = discRp / disc;
			disc = disc * 100;
			$('#disc-pr'+i).val(Math.ceil(disc));
	...
	function hitungRupiah(i){
		...
		if($('#jml'+i).val() !== '' && $('#hna'+i).val() !== ''){
			var jumlah = parseInt($('#jml'+i).val());
			var hna = currencyToNumber($('#hna'+i).val());
			var discRp = parseInt($('#disc-pr'+i).val());
				
			x = hna*jumlah;
			y =  discRp / 100;
			disc = x * y;
			
			$('#disc-rp'+i).val(money_format(disc));
	...
	function simpanPenerimaan() {
		...
		$.ajax({
			type: 'POST',
			url: '<?= base_url("inventori_gizi/api/inventori_gizi/simpan_penerimaan") ?>' + update,
			data: $('#form-penerimaan-inventori').serialize(),
			cache: false,
			dataType: 'JSON',
			beforeSend: function() {
				showLoader()
	...
	function editPenerimaan(detail) {
		...
		$('#tanggal').val(value[4])
		$('#pabrik-auto').val(value[5])
		$('#s2id_pabrik-auto a .select2c-chosen').html(value[6])
	...
	function showDetailPenerimaan(id_penerimaan) {
		$.ajax({
			type: 'GET',
			url: '<?= base_url('inventori_gizi/api/inventori_gizi/get_detail_penerimaan') ?>/id/' + id_penerimaan,
			cache: false,
			dataType: 'JSON',
			beforeSend: function() {
				showLoader()
				...
				console.log(data);
					...
					let html = /* html */ `
						<tr class="rows">
							...
							<td><?= form_dropdown('edit_pembayaran[]', $pembayaran, array(), 'id="edit-pembayaran${i}" class="custom-form" style="width:200px"') ?></td>
							<td><input type="text" name="keterangan_pembayaran[]" id="keterangan-pembayaran${i}" value="${data.keterangan_pembayaran !== null ? data.keterangan_pembayaran: ''}" class="custom-form"></td>
					...
					$('#edit-pembayaran' + i).val('');

					$('#edit-pembayaran' + i).val(data.kategori_pembayaran);
				})
			},
			complete: function() {
				hideLoader()
	...
	function deleteDetailPenerimaan(id, el) {
		swal.fire({
			...
			if (result.value) {
				$.ajax({
					type: 'DELETE',
					url: '<?= base_url("inventori_gizi/api/inventori_gizi/delete_detail_penerimaan") ?>' + '/id/' + id,
					cache: false,
					dataType: 'JSON',
					beforeSend: function() {
						showLoader()
