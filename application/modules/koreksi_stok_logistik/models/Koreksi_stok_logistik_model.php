...
class Koreksi_stok_logistik_model extends CI_Model {
	...
	private function sqlKoreksiStokLogistik($search)
	{
	...
	private function _listKoreksiStokLogistik($limit = 0, $start = 0, $search)
	{
		$this->sqlKoreksiStokLogistik($search);
		if ($limit !== 0) :
	...
	function getListKoreksiStokLogistik($limit = 0, $start = 0, $search)
	{
		$result['data'] = $this->_listKoreksiStokLogistik((int)$limit, (int)$start, $search);
		$result['jumlah'] = $this->sqlKoreksiStokLogistik($search)->get()->num_rows();
		$this->db->close();
		return $result;
	}
    ...
    {
        return $this->db->query("select id, waktu from sm_invrt_stok where id_barang = '" . $id . "' and id_stok = '" . $waktu . "'")->row();
    }

    function cekExpired($id)
    {
        return $this->db->query("select ed from sm_invrt_stok where id = '" . $id . "'")->row();
    }
		
 	function simpanKoreksiStokLogistik()
	{
	...
	function getKoreksiStokLogistik($id)
	{
