...
class Stok_opname_logistik_model extends CI_Model {
    ...
    {
        $sql = "select jenis from sm_kategori_barang where irt = 1 group by jenis";
	...
	private function sqlStokLogistik($search)
	{
	...
	private function _listStokLogistik($limit = 0, $start = 0, $search)
	{
		$this->sqlStokLogistik($search);
		if ($limit !== 0) :
	...
	function getListDataStokLogistik($limit = 0, $start = 0, $search)
	{
		$result['data'] = $this->_listStokLogistik($limit, $start, $search);
		$result['jumlah'] = $this->sqlStokLogistik($search)->get()->num_rows();
		$this->db->close();
		return $result;
	}

	function simpanDataStokLogistik()
	{
		$this->db->trans_begin();
		date_default_timezone_set('Asia/Jakarta');
		...
		foreach ($id_barang as $i => $barang) :
			...
			$this->db->insert('sm_invrt_stok', $data);
	...
	function updateDataStokLogistik($id, $param)
	{
		$this->db->trans_begin();
		$this->db->where('id', $id, true)->update('sm_invrt_stok', $param);
		if ($this->db->trans_status() === false) :
			$this->db->trans_rollback();
	...
	function deleteDataStokLogistik($id)
	{
		$this->db->trans_begin();
		$this->db->where('id', $id, true)->delete('sm_invrt_stok');
		if ($this->db->trans_status() === false) :
			$this->db->trans_rollback();
