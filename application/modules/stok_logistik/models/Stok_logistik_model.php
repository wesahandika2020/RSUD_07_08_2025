...
class Stok_logistik_model extends CI_Model {
    ...
    private function _listStok($limit = 0, $start = 0, $search)
    {
		...
		foreach ($result as $i => $val) :
			$stok_awal = $this->db->select("COALESCE(sum(s.masuk) - sum(s.keluar), 0) as awal")->from('sm_invrt_stok as s')->where('s.waktu < ', $val->waktu, true)->where('s.id_barang', (int)$val->id_barang, true)->where('s.ed', $val->ed, true);
