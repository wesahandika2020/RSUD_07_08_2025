<script>
	...
	$(function() {
		...
		$('#btn-export').click(function() {
			window.open('<?= base_url('laporan_layanan_tenaga_medis/export_laporan_layanan_tenaga_medis?') ?>' + $('#form-search-layanan-tenaga-medis').serialize());
		});
	...
	function getLaporanLayananTenagaMedis(page) {
		$.ajax({
			type: 'GET',
			url: '<?= base_url('laporan_layanan_tenaga_medis/api/laporan_layanan_tenaga_medis/get_list_laporan_tenaga_medis') ?>/page/' + page,
				...
				$.each(data.data, function(i, v) {
					...
					let html = /* html */ `
						...
						${'<tr id="show' + v.id + '" class="collapse">'}
							...
							<td style="background-color: #f9e897; font-weight:bold;" class="center">No. RM</td>
							<td style="background-color: #f9e897; font-weight:bold;" class="left">Nama Pasien</td>
							<td style="background-color: #f9e897; font-weight:bold;" class="left">Penjamin</td>
							<td style="background-color: #f9e897; font-weight:bold;" class="center">Tanggal Daftar</td>
						...
						$.each(v.layanan_pendaftaran.filter(data => data), function(key, val) {
							$(function() {
							...
							${'<tr id="show' + v.id + '" class="collapse">'}
								...
								<td style="background-color: #fdf5d0" class="center">${(val.id)}</td>
								<td style="background-color: #fdf5d0" class="left">${(val.nama)}</td>
								<td style="background-color: #fdf5d0" class="left">${(val.penjamin ?? '-')}</td>
								<td style="background-color: #fdf5d0" class="center">${datefmysql(val.tanggal_daftar)}</td>
								<td style="background-color: #fdf5d0" class="center">${datefmysql(val.tanggal_layanan)}</td>
								<td style="background-color: #fdf5d0" class="center">${btnExpand}</td>
							</tr>
							${'<tr id="show' + val.id_layanan_pendaftaran + '" class="collapse collapse-sec">'}
								...
								<td style="background-color: #ffafb6; font-weight:bold;" class="center">Layanan</td>
								<td style="background-color: #ffafb6; font-weight:bold;" class="center">Tarif Layanan</td>
								<td class="right"></td>
							</tr>
							`;

							$.each(val.layanan.filter(data => data), function(kunci, data) {
								html += `
								${'<tr id="show' + val.id_layanan_pendaftaran + '" class="collapse collapse-sec">'}
									<td></td>
									<td></td>
							...
							${'<tr id="show' + val.id_layanan_pendaftaran + '" class="collapse collapse-sec">'}
								<td></td>
								<td></td>
								<td colspan="2" class="right" style="background-color: #FFD147"><b>Total tarif layanan</b></td>
								<td class="center" style="background-color: #FFD147"><b>${number_format(val.total_tarif, 0, ',', '.')} &ensp;</b></td>
								<td></td>
							</tr>
							`;
						});
					...
					let html = /* html */ `
					<tr>
						<td colspan="6" class="right" style="background-color: #FFD147"><b>Total tarif layanan</b></td>
