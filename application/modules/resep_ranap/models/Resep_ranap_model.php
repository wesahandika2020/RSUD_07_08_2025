...
class Resep_ranap_model extends CI_Model
{
    ...
    private function sqlResepRanap($search)
    {
        $this->db->select("DISTINCT ON (lp.id) lp.*, 
                            ...
                            CONCAT_WS(' ', b.nama, 'Kelas', k.nama, 'Bed', ri.no_bed) as bed,
        ...
        if ($search['jenis'] === 'Rawat Inap') :
            $this->db->where("lp.jenis_layanan in ('Rawat Inap')");
            // $this->db->where("pd.tanggal_keluar is NULL");
            $this->db->where(" (pd.tanggal_keluar is NULL OR lp.tindak_lanjut='cco sementara' ) ");
    ...
    private function _listResepRanap($limit, $start, $search)
    {
        $this->sqlResepRanap($search);
        if ($limit !== 0) :
    ...
    function getListDataResepRanap($limit, $start, $search)
    {
        $data = $this->_listResepRanap($limit, $start, $search);
        ...
        $result['data']   = $data;
        $result['jumlah'] = $this->sqlResepRanap($search)->get()->num_rows();
        $this->db->close();
        return $result;
    }
}
