<script>
	$(function() {
		getListPemakaianBarangGizi(1)
		
		$('#btn-add').click(function() {
			resetData()
			$('#modal-pemakaian-barang-gizi').modal('show')
		})

		$('#barang-auto').select2c({
            width: '100%',
            ajax: {
                url: "<?= base_url('pemakaian_barang_gizi/api/pemakaian_barang_gizi/barang_with_stok_gizi') ?>",
                dataType: 'json',
                quietMillis: 100,
	...
	function getListPemakaianBarangGizi(page) {
		$('#page-now').val(page)
		$.ajax({
			type:'GET',
			url: '<?php echo base_url('pemakaian_barang_gizi/api/pemakaian_barang_gizi/get_list_pemakaian_barang_gizi/page/') ?>' + page,
			...
			success: function(data) {
				if ((page - 1) & (data.data.length === 0)) {
					getListPemakaianBarangGizi(page - 1)
					return false;
				}
					...
					let html = /* html */ `
						<tr>
							...
							<td class="right wrapper">
								<button type="button" class="btn btn-secondary btn-xs" onclick="deletePemakaianBarangGizi(${val.id}, ${data.page})"><i class="fas fa-trash-alt mr-1"></i>Hapus</button>
							</td>
						</tr>
					`;
	...
	function paging(page) {
		getListPemakaianBarangGizi(page)
	}
	...
	function simpanPemakaianBarangGizi() {
		...
		swal.fire({
			title: 'Konfirmasi Simpan',
 			html: "Apakah anda yakin akan menyimpan data pemakaian barang gizi ini ?",
			icon: 'question',
			...
			if (result.value) {
				$.ajax({
					type: 'POST',
					url: '<?= base_url("pemakaian_barang_gizi/api/pemakaian_barang_gizi/simpan_pemakaian_barang_gizi") ?>',
					data: $('#form-pemakaian-barang-gizi').serialize(),
					cache: false,
					dataType: 'JSON',
					beforeSend: function() {
						showLoader()
					...
					success: function(data) {
						...
						if (data.status) {
							type = 'Success';
							getListPemakaianBarangGizi(1)
							$('#modal-pemakaian-barang-gizi').modal('hide')
						}
					...
					error: function(e) {
						messageCustom('Terjadi Kesalahan, Gagal menyimpan data pemakaian barang gizi', 'Error')
					},
				})
			}
		})
	...
	function deletePemakaianBarangGizi(id, page) {
		swal.fire({
			title: 'Konfirmasi Hapus',
 			html: "Apakah anda yakin akan menghapus data pemakaian barang gizi ini ?",
			icon: 'question',
			showCancelButton: true,
			buttonsStyling: true,
			...
			if (result.value) {
				$.ajax({
					type: 'DELETE',
					url: '<?= base_url("pemakaian_barang_gizi/api/pemakaian_barang_gizi/delete_pemakaian_barang_gizi/id/") ?>' + id,
					cache: false,
					dataType: 'JSON',
					beforeSend: function() {
						showLoader()
					...
					error: function(e) {
						messageCustom('Terjadi Kesalahan, Gagal menghapus data pemakaian barang gizi', 'Error')
					},
				})
			}
		})
