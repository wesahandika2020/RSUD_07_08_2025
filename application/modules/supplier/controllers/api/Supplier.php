...
class Supplier extends REST_Controller
{
    ...
    function get_supplier_get()
    {
        if (!$this->get('id', true)) :
            $this->response(NULL, REST_Controller::HTTP_BAD_REQUEST); // (400)
        endif;

        $data['data'] = $this->m_supplier->getDataSupplierById($this->get('id', true));
        $data['page'] = 1;
    ...
    function get_list_supplier_get()
    {
        if (!$this->get('page')) :
            $this->response(NULL, REST_Controller::HTTP_BAD_REQUEST); // (400)
        endif;
        ...
        $data           = $this->m_supplier->getListDataSupplier($start, $this->limit, $search);
    ...
    function delete_supplier_delete()
    {
        $id = $this->get('id', true);
        if ($id <= 0) :
            $this->response(NULL, REST_Controller::HTTP_BAD_REQUEST); // (400)
        ...
        $this->m_supplier->deleteDataSupplier($id);
        $message = [
    ...
    function simpan_supplier_post()
    {
        ...
        if (!$this->get('id')) :
            $this->m_supplier->simpanDataSupplier($data);
            $message = [
                'status' => TRUE,
            ...
            $id = $this->m_supplier->updateDataSupplier($data, $this->get('id'));
            $message = [
                'id'     => convert_hash($id),
                'status' => true,
                'token'  => $this->security->get_csrf_hash()
