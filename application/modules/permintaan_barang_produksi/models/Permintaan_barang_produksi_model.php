...
class Permintaan_barang_produksi_model extends CI_Model
{
	...
	private function sqlBarangUnitSisaStok($search)
	{
	...
	private function _listBarangUnitSisaStok($limit = 0, $start = 0, $search)
	{
		$this->sqlBarangUnitSisaStok($search);
		if ($limit !== 0) :
		...
		foreach ($result as $i => $value) :
			...
			$result[$i]->sisa = $this->db->select("COALESCE(sum(masuk) - sum(keluar), 0.00) as sisa")
			                             ->from('sm_stok')->where('id_barang', $value->id)->where('id_unit', $search['unit'])
			                             ->get()->row()->sisa;
			// query cari kemasan
	...
	function getListDataBarangUnitSisaStok($limit = 0, $start = 0, $search)
	{
		$result['data'] = $this->_listBarangUnitSisaStok($limit, $start, $search);
		$result['jumlah'] = $this->sqlBarangUnitSisaStok($search)->get()->num_rows();
		return $result;

		$this->db->close();
	}

	function generateKodePermintaanBarang()
	{
		$tahun = date('Y');
		$bulan = date('m');
		$row = $this->db->query("select kode from sm_distribusi where date_part('month', tanggal_permintaan) = ".$bulan." and date_part('year', tanggal_permintaan) = ".$tahun." and kode is not NULL order by kode desc limit 1")->row();
		if ($row) :
	...
	function simpanDataPermintaanBarang()
	{
		$this->db->trans_begin();
		$id = safe_post('id');
		...
		if ($id === '') :
			$data = array(
				...
				'kode' => $this->generateKodePermintaanBarang(),
				'tanggal_permintaan' => $tanggal_permintaan,
				'id_users_permintaan' => $this->session->userdata('id_translucent'),
				'id_unit_asal' => $this->session->userdata('id_unit'),
				'id_unit_tujuan' => $id_unit
			);
			$this->db->insert('sm_distribusi', $data);
			...
			if (is_array($kemasan)) :
				foreach ($kemasan as $i => $val) :
					...
					$data_detail = array(
						...
						'id_kemasan_barang' => $kemasan[$i],
						'hna' => $barang->hna,
						'hpp' => $barang->hpp,
						'jumlah' => $jumlah[$i]
					);
					$this->db->insert('sm_detail_distribusi', $data_detail);
				endforeach;
			endif;
			...
			if ($this->db->trans_status() === false) :
				...
				$result['status'] = false;
				$result['message'] = 'Gagal menyimpan data permintaan barang';
			else :
				$this->db->trans_commit();
				$result['status'] = true;
				$result['message'] = 'Berhasil menyimpan data permintaan barang';
			endif;
			$result['action'] = 'add';
		else :
			$data = array(
			...
			$this->db->where('id', $id, true)->update('sm_distribusi', $data);
			$id_distribusi = $id;
			$this->db->where('id_distribusi', $id_distribusi, true)->delete('sm_detail_distribusi');
			if (is_array($kemasan)) :
				foreach ($kemasan as $i => $val) :
					$sql = "select b.hna, b.hpp from sm_barang as b
                            join sm_kemasan_barang as kb on (kb.id_barang = b.id) 
			...
			if ($this->db->trans_status() === false) :
				$this->db->trans_rollback();
				$result['status'] = false;
				$result['message'] = 'Gagal mengubah data permintaan barang';
			else :
				$this->db->trans_commit();
				$result['status'] = true;
				$result['message'] = 'Berhasil mengubah data permintaan barang';
			endif;
	...
	private function sqlPermintaanBarang($search)
	{
		...
		return $this->db->order_by('d.id', 'asc');
	}

	private function _listPermintaanBarang($limit = 0, $start = 0, $search)
	{
		$this->sqlPermintaanBarang($search);
		if ($limit !== 0) :
			$this->db->limit($limit, $start);
		endif;
	...
	function getListDataPermintaanBarang($limit = 0, $start = 0, $search)
	{
		$result['data'] = $this->_listPermintaanBarang($limit, $start, $search);
		$result['jumlah'] = $this->sqlPermintaanBarang($search)->get()->num_rows();
		return $result;
		$this->db->close();
	}

	function getDataPermintaanBarang($id)
	{
		$param = NULL;
		$add_select = NULL;
		if (isset($id) && $id !== '') :
			$param .= " and d.id = '".$id."'";
			$add_select = ", 
                COALESCE((select sum(stk.masuk) - sum(stk.keluar) from sm_stok as stk where stk.id_barang = b.id and stk.ed > '".date("Y-m-d")."' and stk.id_unit = '".$this->session->userdata('id_unit')."'), 0.00) as sisa,
                ...
                COALESCE((select sum(stk.masuk) - sum(stk.keluar) from sm_stok as stk where stk.id_barang = b.id and stk.ed > '".date("Y-m-d")."' and stk.id_unit = u.id), 0.00) as sisa_asal ";
		endif;
	...
	function deleteDataPenerimaanBarang($id)
	{
		$this->db->trans_begin();
		$this->db->where('id', $id, true)->delete('sm_distribusi');
		if ($this->db->trans_status() === false) :
			$this->db->trans_rollback();
			$result['status'] = false;
			$result['message'] = "Gagal menghapus data penerimaan barang!";
		else :
			$this->db->trans_commit();
			$result['status'] = true;
			$result['message'] = "Berhasil menghapus data penerimaan barang!";
		endif;
		return $result;
	}
}
