<script>
	$(function () {
		...
		$('#pencarian-barang').keyup(debounce(function () {
			getListSisaStok(1)
			return false;
		}))
	})
	...
	function getListSisaStok(page) {
		$.ajax({
			type: 'GET',
			url: '<?= base_url('permintaan_barang_produksi/api/permintaan_barang_produksi/get_list_barang_unit_sisa_stok') ?>/page/' + page,
	...
	function simpanPermintaanBarang() {
		$.ajax({
			type: 'POST',
			url: '<?= base_url("permintaan_barang_produksi/api/permintaan_barang_produksi/simpan_permintaan_barang_produksi") ?>',
	...
	function getListPermintaanBarang(page, id_barang) {
		...
		$.ajax({
			type: 'GET',
			url: '<?= base_url('permintaan_barang_produksi/api/permintaan_barang_produksi/get_list_permintaan_barang_produksi') ?>/page/' + page + '/id/' + id,
	...
	function addBarangToListPermintaan(detail) {
		...
		for (let i = 0; i < jml; i++) {
			...
			if (id_kemasan === data[1]) {
				swalAlert('warning', 'Validasi', 'Data barang <b>' + data[0] + '</b> sudah di masukkan di nomor ' + (i + 1) + '!');
				return false;
			}
		}
	...
	function editPermintaanBarang(detail) {
		...
		$.ajax({
			type: 'GET',
			url: '<?= base_url('permintaan_barang_produksi/api/permintaan_barang_produksi/get_permintaan_barang') ?>/id/' + data[0],
			dataType: 'JSON',
			beforeSend: function() {
				showLoader()
			},
	...
	function deletePermintaanBarang(id, page) {
		swal.fire({
			...
			if (result.value) {
				$.ajax({
					type: 'DELETE',
					url: '<?= base_url("permintaan_barang_produksi/api/permintaan_barang_produksi/delete_permintaan_barang") ?>/id/' + id,
					cache: false,
					dataType: 'JSON',
					success: function(data) {
	...
	function printPermintaanBarang(id, page) {
		...
		window.open('<?= base_url() ?>permintaan_barang_produksi/printing_bukti_permintaan_barang?id=' + id,'Cetak Bukti Permintaan Barang','width='+dWidth+', height='+dHeight+', left='+x+',top='+y)
	}
</script>
