<script>
	...
	function simpanChecklistPenerimaanPasienRawatInap() {
		...
		$.ajax({
			type: 'POST',
			url: '<?= base_url("rekam_medis/api/rekam_medis/simpan_checklist_penerimaan_pasien_rawat_inap") ?>',
			...
			success: function(data) {
				if (data.status) {
					...
					window.open('<?= base_url('pemeriksaan_poli/cetak_checklist_penerimaan_pasien_rawat_inap/') ?>' + id, 'Cetak Checklist Penerimaan Pasien Rawat Inap', 'width=' + dWidth + ', height=' + dHeight + ', left=' + x + ', top=' + y);
					showListForm($('#id-pendaftaran-cpri').val(),$('#id-layanan-pendaftaran-cpri').val(), $('#id-pasien-cpri').val());  
				} else {
		...
		<div class="modal-content">
			...
			<div class="modal-body">
				<?= form_open('', 'id=form_checklist_penerimaan_pasien_rawat_inap class="form-horizontal"') ?>
					<input type="hidden" name="id_pasien" id="id-pasien-cpri">
   					<input type="hidden" name="id_pendaftaran" id="id-pendaftaran-cpri">
									...
									<div class="form-checklist-penerimaan-pasien-rawat-inap">
										<table class="table table-no-border table-sm table-striped">
											<tr>
												...
												<td width="48%">
													<input type="radio" name="is_pasien" id="is-pasien-cpri-ya" value="1" class="mr-1">Ya
													<input type="radio" name="is_pasien" id="is-pasien-cpri-tidak" value="0" class="mr-1 ml-2">Tidak
												</td>
											</tr>
											<tr>
