<script>
    ...
    function resetFormFpg() {
        $('#tabel_fpg .body-table').empty();
    }

    function tambahFORM_KEP_124_00(data) {
        ...
        resetFormFpg();
        entriFpg(layanan.id_pendaftaran, layanan.id, bed, action);

    }

    function lihatFORM_KEP_124_00(data) {
        let pasien = data.pasien;
        let layanan = data.layanan;
        let bed = layanan.bangsal + ' kelas ' + layanan.kelas + ' ruang ' + layanan.no_ruang + ' No. Bed ' + layanan.no_bed;
        let action = 'lihat';
    ...
    function editFORM_KEP_124_00(data) {
        let pasien = data.pasien;
        let layanan = data.layanan;
        let bed = layanan.bangsal + ' kelas ' + layanan.kelas + ' ruang ' + layanan.no_ruang + ' No. Bed ' + layanan.no_bed;
        let action = 'edit';
    ...
    function entriFpg(id_pendaftaran, id_layanan_pendaftaran, bed, action) {
        $('#btn_simpan_fpg').hide();
        if (action !== 'lihat' ) {
            $('#btn_simpan_fpg').show();
        } else {
        ...
        $.ajax({
			...
			success: function(data) {
				...
				resetFormFpg();
                $('#fpg_id_layanan_pendaftaran').val(id_layanan_pendaftaran);
                $('#fpg_id_pendaftaran').val(id_pendaftaran);
                $('#fpg_id_bed, #fpg_bed').val(bed).text(bed);
                if (data.pasien !== null) {
                    $('#id_pasien, #fpg_no_rm').val(data.pasien.id_pasien).text(data.pasien.id_pasien);
                    $('#fpg_nama_pasien').text(data.pasien.nama);
                    $('#fpg_tanggal_lahir').text((data.pasien.tanggal_lahir !== null ? datefmysql(data.pasien.tanggal_lahir) : '-') + (' (' + hitungUmur(data.pasien.tanggal_lahir) + ')'));
                    $('#fpg_jenis_kelamin').text((data.pasien.kelamin === 'L' ? 'Laki - laki' : 'Perempuan'));
                    if (data.pasien.alergi !== null) {
                        $('#fpg_riwayat_alergi').val(data.pasien.alergi).attr('readonly', true)
                    };
                    $('#fpg_alamat').text(data.pasien.alamat);
                }
                $('.fpg_logo_alergi').hide();
                if (data.profil !== null) {
                ...
                // grafik lenght
                if (typeof data.grafik_fpg !== 'undefined' && data.grafik_fpg !== null) {
                    // Mengganti nilai null pada lingkar_kepala dengan 0
                    data.grafik_fpg.forEach(function (v) {
                    ...
                    grafik1(data.grafik_fpg, id_layanan_pendaftaran, action);
                    grafik2(data.grafik_fpg, id_layanan_pendaftaran, action);
                    grafik3(data.grafik_fpg, id_layanan_pendaftaran, action);

                    $.each(data.grafik_fpg, function (i, v) {
                        console.log(v);
                        let html = /* html */ `
                        <tr class="row-in-${i + 1}">
                        ...
                        `;
                        $('#tabel_fpg .body-table').append(html);
                    });
                } else {
                    $('#tabel_fpg .body-table').empty();
                }

				$('#modal_fpg').modal('show');
			},
			complete: function() {
				hideLoader();
			},
    ...
    function grafik1(data, id_layanan_pendaftaran, action) {
        ...
        chart1 = Highcharts.chart('grafik_fpg1', options);
    }
    ...
    function grafik2(data, id_layanan_pendaftaran, action) {
        ...
        chart2 = Highcharts.chart('grafik_fpg2', options);
    }
    ...
    function grafik3(data, id_layanan_pendaftaran, action) {
        ...
        chart3 = Highcharts.chart('grafik_fpg3', options);
    }

    function simpanFpg() {
        $.ajax({
            type: 'POST',
            url: '<?= base_url("form_rekam_medis/api/entri_keperawatan/simpan_fpg") ?>',
            data: $('#form_fpg').serialize(),
            cache: false,
            dataType: 'JSON',
            beforeSend: function() {
                showLoader();
            ...
            success: function(data) {
                ...
                if (data.respon !== null) {
                    ...
                    if (data.status) {
                        messageAddSuccess();
                        $('#modal_fpg').modal('hide');
                        showListForm($('#fpg_id_pendaftaran').val(), $('#fpg_id_layanan_pendaftaran').val(), $('#fpg_id_pasien').val());
                    } else {
                        messageAddFailed();
                    }
                }
