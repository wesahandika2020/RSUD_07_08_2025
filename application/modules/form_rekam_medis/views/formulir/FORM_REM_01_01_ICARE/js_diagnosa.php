<script>
    ...
    function simpanDataPemeriksaanResume() {
        $.ajax({
            type: 'POST',
            url: '<?= base_url("pelayanan/api/pelayanan/simpan_edit_diagnosa_icare") ?>',
            ...
            success: function(data) {
                ...
                if (data.status) {
                    ...
                    let id_pendaftaran = $('#rm-id-pendaftaran').val();

                    let id_layanan = $('#rm_id_layanan_pendaftaran').val();
                    let id_pasien = $('#ek_id_pasien').val();

                    if ($('#action').val() == 'tambah') {
                        tambahFormulirBaru('FORM_REM_01_01_ICARE', id_pendaftaran, id_layanan, id_pasien)
                    } else if (($('#action').val() == 'edit')) {
                        editFormulir('FORM_REM_01_01_ICARE', id_pendaftaran, id_layanan, id_pasien)
                    }
    ...
    function addDiagnosaIcareResume() {
        ...
        console.log($('#dokter-diagnosa-resume').val());

        if ($('#jenis-kasus-resume').val() === '') {
            syams_validation('#jenis-kasus-resume', 'Jenis kasus harus dipilih.');
            return false;
        }
        ...
        var diagnosaKlinik = $('#diagnosa-klinik-resume').val();
        var jenisKasus = $('#jenis-kasus-resume').val();
        ...
        $('#s2id_golongan-sebab-sakit-resume a .select2c-chosen').html('');

        $('#diagnosa-banding-resume').val('');
        $('.copy1').remove();
        $('.copy2').remove();
        $('.copy3').remove();
        $('.copy4').remove();

        $('#diagnosa-klinik-resume').val('');
        $('#jenis-kasus-resume').val('');
