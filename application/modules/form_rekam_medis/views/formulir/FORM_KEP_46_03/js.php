...
<script>
	...
	function lihatFORM_KEP_46_03(data) {
		let pasien = data.pasien;
		let layanan = data.layanan;
		let action = 'lihat';

		entriAsuhanKeperawatan(layanan.id_pendaftaran, layanan.id, action);
	}

	function editFORM_KEP_46_03(data) {
		let pasien = data.pasien;
		let layanan = data.layanan;
		let action = 'edit';
	...
	function tambahFORM_KEP_46_03(data) {
		let pasien = data.pasien;
		let layanan = data.layanan;
		let action = 'tambah';
	...
	function entriAsuhanKeperawatan(id_pendaftaran, id_layanan_pendaftaran, action) {
		$('#btn-simpan').hide();
	...
	function simpanAsuhanKeperawatan() {
		...
		$.ajax({
			...
			success: function(data) {
				...
				showListForm($('#id-pendaftaran').val(), $('#id-layanan-pendaftaran').val(), $('#id-pasien').val());
			},
			complete: function() {
				hideLoader()
			},
	...
	function editAsuhanKeperawatan(id, obj) {
		$.ajax({
			...
			success: function(data) {
				...
				if (keluhan_utama.sesak_nafas !== '') {
					$('#ku-sesak-nafas').prop('checked', true)
				}
				if (keluhan_utama.mual !== '') {
					$('#ku-mual').prop('checked', true)
				}
				if (keluhan_utama.muntah !== '') {
					$('#ku-muntah').prop('checked', true)
				}
				if (keluhan_utama.gatal !== '') {
					$('#ku-gatal').prop('checked', true)
				}
				...
				if (riwayat_penyakit_dahulu.asma !== '') {
					$('#rpd-asma').prop('checked', true)
				}
				if (riwayat_penyakit_dahulu.hipertensi !== '') {
					$('#rpd-hipertensi').prop('checked', true)
				}
				if (riwayat_penyakit_dahulu.jantung !== '') {
					$('#rpd-jantung').prop('checked', true)
				}
				if (riwayat_penyakit_dahulu.diabetes !== '') {
					$('#rpd-diabetes').prop('checked', true)
				}
				...
				if (riwayat_penyakit_keluarga.asma !== '') {
					$('#rpk-asma').prop('checked', true)
				}
				if (riwayat_penyakit_keluarga.hipertensi !== '') {
					$('#rpk-hipertensi').prop('checked', true)
				}
				if (riwayat_penyakit_keluarga.jantung !== '') {
					$('#rpk-jantung').prop('checked', true)
				}
				if (riwayat_penyakit_keluarga.diabetes !== '') {
					$('#rpk-diabetes').prop('checked', true)
				}
				...
				// if (kebiasaan.merokok.hasil === '1') {
				// 	$('#kebiasaan-merokok-ya').prop('checked', true);
				// 	$('#kebiasaan-merokok-ya-lain-lain').val(kebiasaan.merokok.ket_hasil).show();
				// } else {
				// 	$('#kebiasaan-merokok-tidak').prop('checked', true);
				// 	$('#kebiasaan-merokok-ya-lain-lain').val('').hide();
				// }
				// if (kebiasaan.narkoba.hasil === '1') {
				// 	$('#kebiasaan-narkoba-ya').prop('checked', true);
				// 	$('#kebiasaan-narkoba-ya-lain-lain').val(kebiasaan.narkoba.ket_hasil).show();
				// } else {
				// 	$('#kebiasaan-narkoba-tidak').prop('checked', true);
				// 	$('#kebiasaan-narkoba-ya-lain-lain').val('').hide();
				// }
				// if (kebiasaan.alkohol.hasil === '1') {
				// 	$('#kebiasaan-alkohol-ya').prop('checked', true);
				// 	$('#kebiasaan-alkohol-ya-lain-lain').val(kebiasaan.alkohol.ket_hasil).show();
				// } else {
				// 	$('#kebiasaan-alkohol-tidak').prop('checked', true);
				// 	$('#kebiasaan-alkohol-ya-lain-lain').val('').hide();
				// }

				var kebiasaan = JSON.parse(data.kebiasaan);

				// Reset semua radio button di awal agar tidak ke-centang otomatis saat edit
				$('input[name="kebiasaan_merokok"], input[name="kebiasaan_narkoba"], input[name="kebiasaan_alkohol"]').prop('checked', false);

				// Reset dan sembunyikan input teks di awal
				$('#kebiasaan-merokok-ya-lain-lain, #kebiasaan-narkoba-ya-lain-lain, #kebiasaan-alkohol-ya-lain-lain').val('').hide();

				// Set radio button dan input teks sesuai data
				if (kebiasaan.merokok.hasil === '1') {
					$('#kebiasaan-merokok-ya').prop('checked', true);
					$('#kebiasaan-merokok-ya-lain-lain').val(kebiasaan.merokok.ket_hasil).show();
				} else if (kebiasaan.merokok.hasil === '0') {
					$('#kebiasaan-merokok-tidak').prop('checked', true);
				}
				...
				if (kebiasaan.narkoba.hasil === '1') {
					$('#kebiasaan-narkoba-ya').prop('checked', true);
					$('#kebiasaan-narkoba-ya-lain-lain').val(kebiasaan.narkoba.ket_hasil).show();
				} else if (kebiasaan.narkoba.hasil === '0') {
					$('#kebiasaan-narkoba-tidak').prop('checked', true);
				}
				...
				if (kebiasaan.alkohol.hasil === '1') {
					$('#kebiasaan-alkohol-ya').prop('checked', true);
					$('#kebiasaan-alkohol-ya-lain-lain').val(kebiasaan.alkohol.ket_hasil).show();
				} else if (kebiasaan.alkohol.hasil === '0') {
					$('#kebiasaan-alkohol-tidak').prop('checked', true);
				}
				...
				// Reset dulu biar nggak ada yang kepilih sebelum dicek
				$('input[name="alergi"]').prop('checked', false);
				$('#alergi-ya-lain-lain').val('').removeClass('d-inline-block').hide();

				if (alergi.hasil === 'Tidak') {
					$('#alergi-tidak').prop('checked', true);
				} else if (alergi.hasil === 'Tidak Tahu') {
					$('#alergi-tidak-tahu').prop('checked', true);
				} else if (alergi.hasil === 'Ya') {
					$('#alergi-ya').prop('checked', true);
					$('#alergi-ya-lain-lain').val(alergi.ket_hasil).addClass('d-inline-block').show();
				}
				...
				} else if (data.hubungan_pasien_dengan_keluarga === 'Baik') {
					$('#hubungan-keluarga-baik').prop('checked', true);
				} else {
					// Jika datanya kosong atau tidak sesuai, reset pilihan (opsional)
					$('input[name="hubungan_keluarga"]').prop('checked', false);
				}
				...
				if (status_psikologis.tenang !== '') {
					$('#sp-tenang').prop('checked', true)
				}
				if (status_psikologis.cemas !== '') {
					$('#sp-cemas').prop('checked', true)
				}
				if (status_psikologis.takut !== '') {
					$('#sp-takut').prop('checked', true)
				}
				if (status_psikologis.marah !== '') {
					$('#sp-marah').prop('checked', true)
				}
				if (status_psikologis.sedih !== '') {
					$('#sp-sedih').prop('checked', true)
				}
				if (status_psikologis.bunuh_diri !== '') {
					$('#sp-bunuh-diri').prop('checked', true)
				}
				...
				// start konjungtiva
				if (data.konjungtiva === '0') {
					$('#konjungtiva-tidak').prop('checked', true);
				} else if (data.konjungtiva === '1') {
					$('#konjungtiva-ya').prop('checked', true);
				} else {
					// Jika datanya kosong atau tidak sesuai, reset pilihan (opsional)
					$('input[name="konjungtiva"]').prop('checked', false);
				}
				...
				if (ekstermitas.tidak_edema !== '') {
					$('#tidak-edema').prop('checked', true)
				}
				if (ekstermitas.dehidrasi !== '') {
					$('#dehidrasi').prop('checked', true)
				}
				if (ekstermitas.edema !== '') {
					$('#edema').prop('checked', true)
				}
				if (ekstermitas.edema_anasarka !== '') {
					$('#edema-anasarka').prop('checked', true)
				}
				if (ekstermitas.pucat_dingin !== '') {
					$('#pucat-dingin').prop('checked', true)
				}
				// end ekstermitas
				...
				if (akses_vaskuler.av_fistula !== '') {
					$('#av-fistula').prop('checked', true)
				}
				if (akses_vaskuler.hd_kateter !== '') {
					$('#hd-kateter').prop('checked', true)
				}
				if (akses_vaskuler.subciavia !== '') {
					$('#subciavia').prop('checked', true)
				}
				if (akses_vaskuler.jugular !== '') {
					$('#jugular').prop('checked', true)
				}
				if (akses_vaskuler.femoral !== '') {
					$('#femoral').prop('checked', true)
				}

				// lokasi
				if (akses_vaskuler.lokasi.kanan !== '') {
					$('#lokasi-kanan').prop('checked', true)
				}
				if (akses_vaskuler.lokasi.kiri !== '') {
					$('#lokasi-kiri').prop('checked', true)
				}
				// end akses vaskuler
				...
				if (diag.diag_keperawatan_1 !== '') {
					$('#diag-keperawatan-1').prop('checked', true)
				}
				if (diag.diag_keperawatan_2 !== '') {
					$('#diag-keperawatan-2').prop('checked', true)
				}
				if (diag.diag_keperawatan_3 !== '') {
					$('#diag-keperawatan-3').prop('checked', true)
				}
				if (diag.diag_keperawatan_4 !== '') {
					$('#diag-keperawatan-4').prop('checked', true)
				}
				if (diag.diag_keperawatan_5 !== '') {
					$('#diag-keperawatan-5').prop('checked', true)
				}
				if (diag.diag_keperawatan_6 !== '') {
					$('#diag-keperawatan-6').prop('checked', true)
				}
				if (diag.diag_keperawatan_7 !== '') {
					$('#diag-keperawatan-7').prop('checked', true)
				}
				if (diag.diag_keperawatan_8 !== '') {
					$('#diag-keperawatan-8').prop('checked', true)
				}
				...
				if (inv.intervensi_keperawatan_1 !== '') {
					$('#intervensi-keperawatan-1').prop('checked', true)
				}
				if (inv.intervensi_keperawatan_2 !== '') {
					$('#intervensi-keperawatan-2').prop('checked', true)
				}
				if (inv.intervensi_keperawatan_3 !== '') {
					$('#intervensi-keperawatan-3').prop('checked', true)
				}
				if (inv.intervensi_keperawatan_4 !== '') {
					$('#intervensi-keperawatan-4').prop('checked', true)
				}
				if (inv.intervensi_keperawatan_5 !== '') {
					$('#intervensi-keperawatan-5').prop('checked', true)
				}
				if (inv.intervensi_keperawatan_6 !== '') {
					$('#intervensi-keperawatan-6').prop('checked', true)
				}
				if (inv.intervensi_keperawatan_7 !== '') {
					$('#intervensi-keperawatan-7').prop('checked', true)
				}
				if (inv.intervensi_keperawatan_8 !== '') {
					$('#intervensi-keperawatan-8').prop('checked', true)
				}
				if (inv.intervensi_keperawatan_9 !== '') {
					$('#intervensi-keperawatan-9').prop('checked', true)
				}
				if (inv.intervensi_keperawatan_10 !== '') {
					$('#intervensi-keperawatan-10').prop('checked', true)
				}
				if (inv.intervensi_keperawatan_11 !== '') {
					$('#intervensi-keperawatan-11').prop('checked', true)
				}
				...
				if (inv.intervensi_keperawatan_13 !== '') {
					$('#intervensi-keperawatan-13').prop('checked', true)
				}
				...
				if (inv_kolab.program_hd !== '') {
					$('#inv-program-hd').prop('checked', true)
				}
				if (inv_kolab.transfusi_darah !== '') {
					$('#inv-transfusi-darah').prop('checked', true)
				}
				if (inv_kolab.kolaborasi_diit !== '') {
					$('#inv-kolaborasi-diit').prop('checked', true)
				}
				if (inv_kolab.pemberian_ga_glueonas !== '') {
					$('#inv-pemberian-ca-glueonas').prop('checked', true)
				}
				if (inv_kolab.pemberian_antipiretik !== '') {
					$('#inv-pemberian-antipiretik').prop('checked', true)
				}
				if (inv_kolab.pemberian_analgetik !== '') {
					$('#inv-pemberian-analgetik').prop('checked', true)
				}
				if (inv_kolab.pemberian_erytropoetin !== '') {
					$('#inv-pemberian-erytropoetin').prop('checked', true)
				}
				if (inv_kolab.pemberian_preparat_besi !== '') {
					$('#inv-preparat-besi').prop('checked', true)
				}
				if (inv_kolab.obat_obat_emergensi !== '') {
					$('#inv-obat-obat-emergensi').prop('checked', true)
				}
				...
				if (im.inisiasi !== '') {
					$('#im-inisiasi').prop('checked', true)
				}
				if (im.akut !== '') {
					$('#im-akut').prop('checked', true)
				}
				if (im.rutin !== '') {
					$('#im-rutin').prop('checked', true)
				}
				if (im.pre_op !== '') {
					$('#im-pre-op').prop('checked', true)
				}
				if (im.sled !== '') {
					$('#im-sled').prop('checked', true)
				}
				...
				if (dialisat.bicarbonat !== '') {
					$('#dialisat-bicarbonat').prop('checked', true)
				}
				if (dialisat.condusctivity !== '') {
					$('#dialisat-condusctivity').prop('checked', true)
				}
				if (dialisat.temperatur !== '') {
					$('#dialisat-temperatur').prop('checked', true)
				}
				...
				if (ps.masalah_akses !== '') {
					$('#ps-masalah-akses').prop('checked', true)
				}
				if (ps.pendarahan !== '') {
					$('#ps-pendarahan').prop('checked', true)
				}
				if (ps.first_use_syndrom !== '') {
					$('#ps-use-syndrom').prop('checked', true)
				}
				if (ps.sakit_kepala !== '') {
					$('#ps-sakit-kepala').prop('checked', true)
				}
				if (ps.mual_muntah !== '') {
					$('#ps-mual-muntah').prop('checked', true)
				}
				if (ps.kram_otot !== '') {
					$('#ps-kram-otot').prop('checked', true)
				}
				if (ps.hiperkalemia !== '') {
					$('#ps-hiperkalemia').prop('checked', true)
				}
				if (ps.hipotensi !== '') {
					$('#ps-hipotensi').prop('checked', true)
				}
				if (ps.hipertensi !== '') {
					$('#ps-hipertensi').prop('checked', true)
				}
				if (ps.nyeri_dada !== '') {
					$('#ps-nyeri-dada').prop('checked', true)
				}
				if (ps.aritmia !== '') {
					$('#ps-aritmia').prop('checked', true)
				}
				if (ps.gatal_gatal !== '') {
					$('#ps-gatal-gatal').prop('checked', true)
				}
				if (ps.demam !== '') {
					$('#ps-demam').prop('checked', true)
				}
				if (ps.menggigil !== '') {
					$('#ps-menggigil').prop('checked', true)
				}
				if (ps.lain_lain !== '') {
	...
	function showTindakanKeperawatan(pre_hd, intra_hd, post_hd) {
		if (pre_hd !== null) {
			...
			if (pre_hd.paraf !== null) {
				$('#pre-paraf').prop('checked', true)
			}
			// end pre hd
		}
		...
		if (intra_hd !== null) {
			...
			$.each(intra_hd, function(i, v) {
				...
				if (v.paraf !== null) {
					$('#intra-paraf' + i).prop('checked', true)
				}
				// end intra hd
			});
		}
		...
		if (post_hd !== null) {
			...
			if (post_hd.paraf !== null) {
				$('#post-paraf').prop('checked', true)
			}
			// post
