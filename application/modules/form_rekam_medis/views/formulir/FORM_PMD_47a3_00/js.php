<script>
    ...
    function resetFormWfg() {
        $('#tabel_wfg .body-table').empty();
    }

    function tambahFORM_PMD_47a3_00(data) {
        ...
        resetFormWfg();
        entriWfg(layanan.id_pendaftaran, layanan.id, bed, action);
 
    }

    function lihatFORM_PMD_47a3_00(data) {
        let pasien = data.pasien;
        let layanan = data.layanan;
        let bed = layanan.bangsal + ' kelas ' + layanan.kelas + ' ruang ' + layanan.no_ruang + ' No. Bed ' + layanan.no_bed;
        let action = 'lihat';
    ...
    function editFORM_PMD_47a3_00(data) {
        let pasien = data.pasien;
        let layanan = data.layanan;
        let bed = layanan.bangsal + ' kelas ' + layanan.kelas + ' ruang ' + layanan.no_ruang + ' No. Bed ' + layanan.no_bed;
        let action = 'edit';
    ...
    function entriWfg(id_pendaftaran, id_layanan_pendaftaran, bed, action) {
        $('#btn_simpan_wfg').hide();
        if (action !== 'lihat' ) {
            $('#btn_simpan_wfg').show();
        } else {
        ...
        $.ajax({
			...
			success: function(data) {
				...
				resetFormWfg();
                $('#wfg_id_layanan_pendaftaran').val(id_layanan_pendaftaran);
                $('#wfg_id_pendaftaran').val(id_pendaftaran);
                $('#wfg_id_bed, #wfg_bed').val(bed).text(bed);
                if (data.pasien !== null) {
                    $('#id_pasien, #wfg_no_rm').val(data.pasien.id_pasien).text(data.pasien.id_pasien);
                    $('#wfg_nama_pasien').text(data.pasien.nama);
                    $('#wfg_tanggal_lahir').text((data.pasien.tanggal_lahir !== null ? datefmysql(data.pasien.tanggal_lahir) : '-') + (' (' + hitungUmur(data.pasien.tanggal_lahir) + ')'));
                    $('#wfg_jenis_kelamin').text((data.pasien.kelamin === 'L' ? 'Laki - laki' : 'Perempuan'));
                    if (data.pasien.alergi !== null) {
                        $('#wfg_riwayat_alergi').val(data.pasien.alergi).attr('readonly', true)
                    };
                    $('#wfg_alamat').text(data.pasien.alamat);
                }
                $('.wfg_logo_alergi').hide();
                if (data.profil !== null) {
                ...
                // grafik length
                if (typeof data.grafik_weight_girls !== 'undefined' && data.grafik_weight_girls !== null) {
                    // Mengganti nilai null pada berat_badan dengan 0
                    data.grafik_weight_girls.forEach(function (v) {
                        v.berat_badan = (v.berat_badan !== null) ? v.berat_badan : 0;
                    });

                    grafik(data.grafik_weight_girls, id_layanan_pendaftaran, action);

                    $.each(data.grafik_weight_girls, function (i, v) {
                        let html = /* html */ `
                        <tr class="row-in-${i + 1}">
                        ...
                        `;
                        $('#tabel_wfg .body-table').append(html);
                    });
                } else {
                    $('#tabel_wfg .body-table').empty();
                }

				$('#modal_wfg').modal('show');
			},
			complete: function() {
				hideLoader();
			},
    ...
    function grafik(data, id_layanan_pendaftaran, action) {
        ...
        chart = Highcharts.chart('grafik_wfg', options);
    }

    function simpanWfg() {
        $.ajax({
            type: 'POST',
            url: '<?= base_url("form_rekam_medis/api/entri_keperawatan/simpan_wfg") ?>',
            data: $('#form_wfg').serialize(),
            cache: false,
            dataType: 'JSON',
            beforeSend: function() {
                showLoader();
            ...
            success: function(data) {
                ...
                if (data.respon !== null) {
                    ...
                    if (data.status) {
                        messageAddSuccess();
                        $('#modal_wfg').modal('hide');
                        showListForm($('#wfg_id_pendaftaran').val(), $('#wfg_id_layanan_pendaftaran').val(), $('#wfg_id_pasien').val());
                    } else {
                        messageAddFailed();
                    }
                }
