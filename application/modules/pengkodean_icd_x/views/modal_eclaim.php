<script>
    function konfirmasiSimpanEklaim() {
        ...
        if ($('#no_sep_eclaim').val() === '') {
            syams_validation('#no_sep_eclaim', 'SEP harus diisi.');
            return false;
        }
        if ($('#nomor_kartu').val() === '') {
            syams_validation('#nomor_kartu', 'No. Peserta harus diisi.');
            return false;
        }
        ...
        if ($('#diagnosa').val() == "") {
            ...
            $.ajax({
                type: 'GET',
                 url: '<?= base_url("vclaim_bpjs/api/vclaim_bpjs_v2/get_detail_sep/no_sep/") ?>' + $('#no_sep_eclaim').val(),
                cache: false,
                dataType: 'JSON',
                beforeSend: function() {
                    showLoader();
                ...
                success: function(data) {
                    if (data.metaData.code !== '200') {
                        syams_validation('#no_sep_eclaim', 'SEP Tidak ditemukan!');
                        messageCustom("SEP Tidak ditemukan! Mohon Perbaiki no. SEP.", 'Error');
                        return false;

                    } else {
                        let jenisLayanan = data.response.jnsPelayanan;
                        let tglSep = data.response.tglSep;
                        let poli = data.response.poli;
                        let noKartu = data.response.peserta.noKartu;
                        let namaPeserta = data.response.peserta.nama;

                        // $('#jenis_rawat_eclaim').val()
                        let tglMasuk = $('#tanggal_rawat_eclaim').val().slice(0, 10);

                        if ($('#nomor_kartu').val() !== noKartu) {
                            syams_validation('#no_sep_eclaim', 'SEP Tidak sesuai!');
                            syams_validation('#nomor_kartu', 'Periksa No. Kartu!');
                            messageCustom("No. Kartu (" + $('#nomor_kartu').val() + ") dengan yang di SEP (" + noKartu + " - " + namaPeserta + ") Tidak sesuai! Periksa kembali no. SEP dan No. Kartu.", 'Error');
                            return false;

                        } else {

                            if ($('#jenis_rawat_eclaim').val() !== jenisLayanan) {
                                syams_validation('#no_sep_eclaim', 'SEP Tidak sesuai!');
                                messageCustom("SEP Tidak sesuai dengan Jenis Rawat! Periksa kembali no. SEP.", 'Error');
                                return false;
                            } else if (jenisLayanan == "Rawat Jalan") {
                                if (tglSep !== tglMasuk) {
                                    syams_validation('#no_sep_eclaim', 'SEP Tidak sesuai!');
                                    messageCustom("SEP Tidak sesuai dengan Tanggal Kunjungan! Periksa kembali no. SEP.", 'Error');
                                    return false;
                                } else {
                                    simpanDataEkaliam()
                                }
    ...
    function simpanDataEkaliam() {
        $.ajax({
            type: 'POST',
            url: '<?= base_url("pengkodean_icd_x/api/eklaim_v_5_8_x/simpan_data_eklaim") ?>',
                    ...
                    <div class="col-lg-6">
                        <div class="table-responsive">
                            <table class="table table-sm table-striped">
                                ...
                                <tr>
                                    <td width="30%" class="bold">Cara Masuk</td>
                                    <td width="80%">
                                        <?= form_dropdown('cara_masuk', $cara_masuk, array(), 'class="custom-form" id=cara-masuk') ?>
                                    </td>
                                </tr>
                            </table>
                        </div>
                            ...
                            <div class="row" style="margin-top: -30px;">
                                <div class="col-lg-4 right">
                                    <table class="table table-no-border table-sm table-striped" style="margin-top: 15px">
                                        ...
                                        <tr>
                                            <td class="bold right">Sistole</td>
                                            <td class="bold">:</td>
                                            <td>
                                                <div class="input-group">
                                                    <input type="text" name="sistole" id="sistole" class="custom-form clear-input d-inline-block col-lg-9" placeholder="sistole">
                                ...
                                <div class="col-lg-4 center">
                                    <table class="table table-no-border table-sm table-striped" style="margin-top: 15px">
                                        ...
                                        <tr>
                                            <td class="bold right">Diastole</td>
                                            <td class="bold">:</td>
                                            <td>
                                                <div class="input-group">
                                                    <input type="text" name="diastole" id="diastole" class="custom-form clear-input d-inline-block col-lg-9" placeholder="diastole">
