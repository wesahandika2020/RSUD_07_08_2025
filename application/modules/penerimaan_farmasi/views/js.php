<script>
	...
	$(function() {
		...
		$('#btn-add').click(function() {
			reloadData()
			$('#modal-penerimaan-title').html('Form Penerimaan Barang Gudang Farmasi')
	...
	function getListPenerimaan(page, id) {
		...
		$.ajax({
			type: 'GET',
			url: '<?= base_url("inventory/api/inventory/get_list_penerimaan") ?>/page/' + page + '/id/' + id,
			data: $('#form-search').serialize() + '&jenis_barang=Farmasi',
			cache: false,
			dataType: 'JSON',
			beforeSend: function() {
				showLoader()
	...
	function printPenerimaan(id_penerimaan) {
		window.open('<?= base_url() ?>inventory/printing_penerimaan/' + id_penerimaan, 'Cetak Penerimaan','width='+dWidth+', height='+dHeight+', left='+x+',top='+y)
	}
	...
	function deletePenerimaan(id, page, string) {
		...
		swal.fire({
			...
			if (result.value) {
				$.ajax({
					type: 'DELETE',
					url: '<?= base_url("inventory/api/inventory/delete_penerimaan") ?>' + '/id/' + id,
					cache: false,
					dataType: 'JSON',
					beforeSend: function() {
						showLoader()
