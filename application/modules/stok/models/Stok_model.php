...
class Stok_model extends CI_Model {
	...
	private function sqlStok($search)
    {
		...
		// else : 
		// 	$this->db->where('s.id_unit', $this->session->userdata('id_unit'));
		endif;
		if ($search['transaksi'] !== '') :
        ...
        if ($search['jenis'] === 'History') :
			return $this->db->order_by('s.waktu, s.id', 'asc');
		else :
			return $this->db->order_by('s.waktu', 'asc');
        endif;
    ...
    private function _listStok($limit = 0, $start = 0, $search)
    {
		...
		foreach ($result as $i => $val) :
			$stok_awal = $this->db->select("COALESCE(sum(s.masuk) - sum(s.keluar), 0) as awal")->from('sm_stok as s')->where('s.waktu < ', $val->waktu, true)->where('s.id_barang', $val->id_barang, true)->where('s.ed', $val->ed, true);
			if ($search['unit'] !== '') :
				$this->db->where('s.id_unit', $search['unit'], true);
			else :
				$this->db->where('s.id_unit', $this->session->userdata('id_unit'));
