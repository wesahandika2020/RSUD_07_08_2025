<script>
	...
	function getListDistribusi(page) {
		...
		$.ajax({
			type: 'GET',
			url: '<?php echo base_url('distribusi/api/distribusi/get_list_distribusi') ?>/page/' + page,
	...
	function distribusiPermintaan(detail) {
		...
		$.ajax({
			type: 'GET',
			url: '<?php echo base_url('permintaan_barang/api/permintaan_barang/get_permintaan_barang') ?>/id/' + data[0],
			cache: false,
			dataType: 'JSON',
			beforeSend: function() {
				showLoader()
	...
	function simpanDistribusi(page) {
		$.ajax({
			type: 'POST',
			url: '<?php echo base_url('distribusi/api/distribusi/simpan_distribusi') ?>',
	...
	function printDistribusi(id) {
		...
		window.open('<?= base_url() ?>distribusi/printing_bukti_distribusi?id=' + id,'Cetak Bukti Distribusi Barang','width='+dWidth+', height='+dHeight+', left='+x+',top='+y)
	}
</script>