<script>
	...
	function entriEdukasi(id_pendaftaran, id_layanan_pendaftaran) {
		...
		$.ajax({
			type: 'GET',
			url: '<?= base_url("pendaftaran_poli/api/pendaftaran_poli/get_edukasi_pasien") ?>/id/' + id_pendaftaran + '/id_layanan/' + id_layanan_pendaftaran + '/tipe/edukasi',
			cache: false,
			dataType: 'JSON',
			beforeSend: function() {
				showLoader();
	...
	function printEdukasi(id_pendaftaran, id_layanan_pendaftaran) {
		window.open('<?= base_url() ?>pendaftaran_poli/printing_edukasi/' + id_pendaftaran + '/' + id_layanan_pendaftaran, 'Cetak Form Edukasi', 'width=' + dWidth + ', height=' + dHeight + ', left=' + x + ', top=' + y);
	}
</script>
