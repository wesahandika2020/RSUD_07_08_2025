<script>
    ...
    function getListPendaftaran(p) {
        ...
        $.ajax({
            type: 'GET',
            url: '<?= base_url("sep_igd/api/sep_igd/get_list_sep_igd") ?>/page/' + p,
            data: $('#form_search').serialize() + '&jenis_igd=' + $('#jenis_igd').val(),
            cache: false,
            dataType: 'JSON',
            beforeSend: function() {
                showLoader();
            ...
            success: function(data) {
                ...
                $.each(data.data, function(i, v) {
                    ...
                    if (v.no_sep !== null) {
                        nosep = v.no_sep;
                        disable_sep = '';
                        btn_sep = '<a title="Update SEP" class="dropdown-item ' + disable + ' waves-effect waves-light btn-xs" href="javascript:void(0)" onclick="formUpdateSEP(\'' + v.no_sep + '\', \'\', \'igd\')"><i class="fas fa-edit m-r-5"></i>Update SEP</a> ';
                        ...
                        nosep = '';
                        disable_sep = 'disabled';
                        btn_sep = '<a title="Klik untuk membuat SEP" class="dropdown-item waves-effect waves-light btn-xs" href="javascript:void(0)" onclick="pembuatanSEP(\'' + v.id_pasien + '\' ,' + v.id + ', \'' + v.no_polish + '\', \'' + v.kode_bpjs + '\')"><i class="fas fa-plus-circle m-r-5"></i>Buat SEP</a> ';
                        btn_rujukan = '';
                    }
