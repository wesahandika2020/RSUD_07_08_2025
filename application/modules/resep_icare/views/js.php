<script>
	...
	function getListResep(p, id_layanan_pendaftaran) {
		...
		$.ajax({
			type: 'GET',
			url: '<?= base_url("resep_icare/api/resep_icare/get_list_resep_icare") ?>/page/' + p + '/jenis/Intensive Care/id/' + id,
			...
			success: function(data) {
				...
				$.each(data.data, function(i, v) {
					...
					let detail = v.id + '#' + v.id_pasien + '#' + v.nama + '#' + v.dokter + '#' + v.id_dokter + '#' + hitungUmur(v.tanggal_lahir) + '#' + v.jenis_layanan + '#' + v.id_penjamin + '#' + v.penjamin + '#' + v.cara_bayar + '#' + v.telp;
		...
		$.ajax({
			...
			success: function(data) {
				...
				$('#modal-form-resep-label').html(`
                    <b>Form Resep Intensive Care ${title_mode}</b> | No. RM : ${data.id_pasien}, Nama: ${data.pasien}, 
				...
				$.each(data.resep_r, function(i, v) {
                            ...
                            <div id="table-list-resep${v.no_r}" class="input-resep" style="width: 100%; display: flex; flex-direction: column; gap:.5rem">
								<div style="display: flex;gap: 1rem; justify-content: space-between">
									<div style="display: flex;gap: 1rem;align-items: center">
										...
										<button type="button" id="jam-pemberian${v.no_r}" class="btn btn-info btn-xs jam-pemberian ${data.is_terapi_pulang === '0' ? 'hide': ''}">
					...
					$(`#jam-pemberian${v.no_r}`).on('click', function(){
						const $this = $(this);
				...
				try {
					...
					$('#res-resep-icare').html(notification(hasil));
				} catch (e) {
					console.log(e)
				}
			},
