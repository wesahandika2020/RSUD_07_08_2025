...
class Laporan_gizi_model extends CI_Model
{
    ...
    function getListLaporanGizi01($limit, $start, $search)
	{
        ...
        $sql = " WITH dpmp_terakhir AS (
                    SELECT lp.id_pendaftaran, lp.id AS id_layanan_pendaftaran, dm.id AS id_dpmp, to_char(dm.waktu_dpmp, 'YYYY-MM-DD') AS waktu_dpmp,
                    ROW_NUMBER() OVER (PARTITION BY lp.id_pendaftaran ORDER BY dm.waktu_dpmp DESC) AS status
                    FROM sm_dpmp dm
                    JOIN sm_layanan_pendaftaran lp ON dm.id_layanan_pendaftaran = lp.id
                    LEFT JOIN sm_diet_cair dc ON dm.id = dc.id_dpmp
                    WHERE dm.id IS NOT NULL $param
                )
                
                SELECT pd.id, lp.id id_layanan_pendaftaran, lp.tanggal_layanan, dm.waktu_dpmp, ri.id id_ranap, ri.id_bangsal id_bangsal_ranap, ic.id id_ic, ic.id_bangsal id_bangsal_ic,
                case when ri.id is not null then concat((select nama from sm_bangsal where id=ri.id_bangsal),' Ruang ',ri.no_ruang,' Bed ', ri.no_bed)
                when ic.id is not null then concat((select nama from sm_bangsal where id=ic.id_bangsal),' Ruang ',ic.no_ruang,' Bed ', ic.no_bed) 
                when lp.jenis_layanan = 'Hemodialisa' then 'Hemodialisa' else '' end ruangan,
                pd.id_pasien, ps.nama, ps.tanggal_lahir,ps.kelamin,
        ...
        $tabel = "FROM dpmp_terakhir dmt
                    JOIN sm_dpmp dm on dmt.id_dpmp=dm.id
                    LEFT JOIN sm_layanan_pendaftaran lp on dm.id_layanan_pendaftaran=lp.id
                    LEFT JOIN sm_rawat_inap ri ON lp.ID = ri.id_layanan_pendaftaran
                    LEFT JOIN sm_intensive_care ic ON lp.ID = ic.id_layanan_pendaftaran
        ...
        $where  =" WHERE dmt.status = 1 AND lp.status_terlayani<>'Batal' AND lp.tindak_lanjut is null ";
    ...
    function getListLaporanGizi02($limit, $start, $search)
	{
        ...
        //             WHERE dmt.status = 1 AND lp.status_terlayani<>'Batal' AND lp.tindak_lanjut is null 
        //         ";
		...
		$data["data"]           = $this->db->query($sql.$tabel.$where.$param.$order.$limitation)->result();
		$data['jml_data']       = $this->db->query($sql.$tabel.$where.$param.$order)->num_rows();
		$data['jml_data_all']   = $this->db->query($sql2.$tabel.$param.$order)->num_rows();
        $data['periode']        = $this->periode($search) ;
		$data['jenis_diet']     = $jenis_diet_map[$search["jenis_diet"]] ?? '';
		$data['tempat_layanan'] = $tempat_layanan_map[$search["tempat_layanan"]] ?? '';
		$data['ruangan_ranap']  = $search["ruangan_ranap"] <>'' ? $this->db->select('nama')->from('sm_bangsal')->where('id', $search["ruangan_ranap"])->get()->row()->nama : '';
    ...
    function getListLaporanGizi03Makan($search,$type,$field)
	{
        ...
        if($type == 'jenisdiet'){
            $sql = " WITH dpmp_terakhir AS (
                    ...
                    SELECT SUM(CASE WHEN dm.jns_diet_mp = '$field' THEN 1 ELSE 0 END) AS mp,
        ...
        if($type == 'bentukmakan'){
            $sql = " WITH dpmp_terakhir AS (
                ...
                SELECT SUM(CASE WHEN dm.btk_mkn_mp = '$field' THEN 1 ELSE 0 END) AS mp,
    ...
    function getListLaporanGizi03Cair($search,$field)
	{
        ...
        $sql = " WITH dpmp_terakhir AS (
                ...
                SELECT (select nama from sm_diet where id=$getIdDiet) jenis_susu,
    ...
    function getListLaporanGizi04($limit, $start, $search)
	{
        ...
        $sql = "WITH dpmp_terakhir AS (
                ...
                SELECT dmt.id_pendaftaran id , dmt.id_layanan_pendaftaran,lp.tanggal_layanan, dmt.id_dpmp, dmt.waktu_dpmp,
                ri.ID id_ranap,ri.id_bangsal id_bangsal_ranap,ic.ID id_ic,ic.id_bangsal id_bangsal_ic,
                CASE WHEN ri.ID IS NOT NULL THEN concat ( ( SELECT nama FROM sm_bangsal WHERE ID = ri.id_bangsal ), ' Ruang ', ri.no_ruang, ' Bed ', ri.no_bed ) 
                     WHEN ic.ID IS NOT NULL THEN concat ( ( SELECT nama FROM sm_bangsal WHERE ID = ic.id_bangsal ), ' Ruang ', ic.no_ruang, ' Bed ', ic.no_bed ) 
                     WHEN lp.jenis_layanan = 'Hemodialisa' THEN 'Hemodialisa' ELSE'' END ruangan,
                pd.id_pasien,ps.nama,
                dm.ID id_diet_makan,dm.jns_diet_mp,dm.jns_diet_sp,dm.jns_diet_ms,dm.jns_diet_ss,dm.jns_diet_mm,dm.jns_diet_sm,
                dm.btk_mkn_mp,dm.btk_mkn_sp,dm.btk_mkn_ms,dm.btk_mkn_ss,dm.btk_mkn_mm,dm.btk_mkn_sm,dm.mp_makan_pasien,
                dm.sp_makan_pasien,dm.ms_makan_pasien,dm.ss_makan_pasien,dm.mm_makan_pasien,dm.sm_makan_pasien,dm.ket_makan_pasien,
                mp.kode AS mp_kode,ms.kode AS ms_kode,mm.kode AS mm_kode,ss.kode AS ss_kode,sp.kode AS sp_kode,sm.kode AS sm_kode,
                dc.ID id_diet_cair, dc.keterangan_diet_cair AS kdc,dc.dpmp_gram,
                to_char( dc.dpmp_jam_satu, 'HH24' ) dpmp_jam_satu,to_char( dc.dpmp_jam_dua, 'HH24' ) dpmp_jam_dua,to_char( dc.dpmp_jam_tiga, 'HH24' ) dpmp_jam_tiga,
                to_char( dc.dpmp_jam_empat, 'HH24' ) dpmp_jam_empat,to_char( dc.dpmp_jam_lima, 'HH24' ) dpmp_jam_lima,to_char( dc.dpmp_jam_enam, 'HH24' ) dpmp_jam_enam,
                to_char( dc.dpmp_jam_tujuh, 'HH24' ) dpmp_jam_tujuh,to_char( dc.dpmp_jam_delapan, 'HH24' ) dpmp_jam_delapan,
                (( CASE WHEN dc.dpmp_jam_satu IS NOT NULL THEN 1 ELSE 0 END ) + ( CASE WHEN dc.dpmp_jam_dua IS NOT NULL THEN 1 ELSE 0 END ) + 
                ( CASE WHEN dc.dpmp_jam_tiga IS NOT NULL THEN 1 ELSE 0 END ) + ( CASE WHEN dc.dpmp_jam_empat IS NOT NULL THEN 1 ELSE 0 END ) + 
                ( CASE WHEN dc.dpmp_jam_lima IS NOT NULL THEN 1 ELSE 0 END ) + ( CASE WHEN dc.dpmp_jam_enam IS NOT NULL THEN 1 ELSE 0 END ) + 
                ( CASE WHEN dc.dpmp_jam_tujuh IS NOT NULL THEN 1 ELSE 0 END ) + ( CASE WHEN dc.dpmp_jam_delapan IS NOT NULL THEN 1 ELSE 0 END ) ) AS freq ";
        ...
        $where  =" WHERE dmt.status = 1  AND lp.status_terlayani<>'Batal'  AND lp.tindak_lanjut is null AND dc.id is not null";
    ...
    function getListLaporanGizi06($limit, $start, $search)
	{
        ...
        $tabel = " FROM dpmp_terakhir dmt
                ...
                ";
        $where  =" AND dmt.status = 1 AND lp.status_terlayani<>'Batal' AND lp.tindak_lanjut is null ";
