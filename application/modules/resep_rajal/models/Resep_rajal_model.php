...
class Resep_rajal_model extends CI_Model
{
    ...
    private function sqlResepRajal($search)
    {
        ...
        if ($search['jenis'] === 'Rawat Jalan') :
            $this->db->where("lp.jenis_layanan in ('Poliklinik','IGD','Kemoterapi', 'Medical Check Up')");
        endif;
        ...
        if ($search['no_rm'] !== '') :
            $this->db->like("p.id ilike '" . strtolower($search['no_rm']) . "%'");
        endif;
        ...
        if ($search['layanan'] !== '') :
            if ($search['layanan'] == 'IGD') :
                $this->db->where('lp.jenis_layanan', $search['layanan']);
            else :
                $this->db->where('lp.id_unit_layanan', $search['layanan']);
            endif;
        else :
            if ($this->session->userdata('id_unit') == 304) :
    ...
    private function _listResepRajal($limit, $start, $search)
    {
        $this->sqlResepRajal($search);
        if ($limit !== 0) :
    ...
    function getListDataResepRajal($limit, $start, $search)
    {
        $data = $this->_listResepRajal($limit, $start, $search);
        foreach ($data as $i => $val) :
            $this->db->select('p.*, r.tanggal_resep, r.id as id_resep')
        ...
        $result['data']   = $data;
        $result['jumlah'] = $this->sqlResepRajal($search)->get()->num_rows();
        $this->db->close();
        return $result;
    }
}
