<script>
	...
	function getListResep(p, id_layanan_pendaftaran) {
		...
		$.ajax({
			type: 'GET',
			url: '<?= base_url("resep_rajal/api/resep_rajal/get_list_resep_rajal") ?>/page/' + p + '/jenis/Rawat Jalan/id/' + id,
			data: $('#form-search').serialize() + '&keyword=' + $('#keyword').val() + '&layanan=' + $('#layanan').val(),
			cache: false,
			dataType: 'JSON',
			beforeSend: function() {
				showLoader();
			...
			success: function(data) {
				...
				$.each(data.data, function(i, v) {
					...
					if (v.list_resep.length === 0) {
						html += /* html */ `
                                        <button ${disable} type="button" class="btn btn-secondary btn-xs" title="Klik untuk input resep" onclick="inputResep('${detail}')">
					...
					if (v.list_resep.length >= 1) {
						html += /* html */ `
                                    ...
                                    `;
						html += /* html */ `
                                        <button ${disable} type="button" class="btn btn-secondary btn-xs" title="Klik untuk edit resep" data-trigger="focus" data-toggle="popover" data-content="${edit.join(' <br/>')}"><i class="fas fa-edit"></i></button>
                                    `;
						html += /* html */ `
