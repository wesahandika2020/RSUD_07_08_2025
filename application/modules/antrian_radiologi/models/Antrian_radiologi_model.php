...
class Antrian_radiologi_model extends CI_Model
{
	...
	function getRuangRadiologi($q = null)
    {
        $sql = "select * from sm_ruang_radiologi where nama_ruangan ilike ('%$q%') order by nama_ruangan asc ";
        $query = $this->db->query($sql)->result();
        $data =  array();
        $data[''] = '- Semua Ruangan -';
        foreach ($query as $key => $value) :
            $data[$value->id] = $value->nama_ruangan;
        endforeach;
