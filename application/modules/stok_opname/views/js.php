<script>
	$(function() {
		...
		$('#tambahkan').click(function() {
			...
			addToRecordList(id_barang, nama_barang, jumlah, id_kemasan, no_batch, expired, alasan)
		})
	...
	function getListStokOpname(page, id_stok_opname) {
		var id = '';
		...
		$.ajax({
			type: 'GET',
			url: '<?php echo base_url('stok_opname/api/stok_opname/get_list_stok_opname/page/') ?>' + page + '/id/' + id ,
				...
				$.each(data.data, function(i, v) {
					let html = '<tr>'+
									...
									'<td class="right wrapper">'+
										'<button type="button" class="btn btn-secondary btn-xs mr-1" onclick="editStokOpname('+v.id+', \''+v.nama_barang+'\')"><i class="fas fa-edit"></i></button>'+
										'<button type="button" class="btn btn-secondary btn-xs" onclick="hapusStokOpname('+v.id+', '+data.page+')"><i class="fas fa-trash-alt"></i></button>'+
									'</td>'+
								'</tr>';
	...
	function paging(page) {
		getListStokOpname(page)
	}

	function addToRecordList(id_barang, nama_barang, jumlah, id_kemasan, no_batch, expired, alasan) {
		if (id_barang === '') {
		...
		let html = /* html */ `
			<tr class="rows">
				...
				<td class="center">${expired}<input type="hidden" name="ed[]" value="${date2mysql(expired)}" class="ed" id="ed${jml}"></td>
	...
	function konfirmasiSimpanStokOpname(param) {
		if (param === 'add') {
			...
			if ($('#ed-edit').val() === '') {
				syams_validation('#ed-edit', 'Expired date tidak boleh kosong!')
				return false;
			}
		}
	...
	function simpanStokOpname() {
		$.ajax({
			type: 'POST',
			url: '<?php echo base_url('stok_opname/api/stok_opname/simpan_stok_opname') ?>',
	...
	function editStokOpname(id, nama_barang) {
		$.ajax({
			type: 'GET',
			url: '<?php echo base_url('stok_opname/api/stok_opname/get_stok_opname') ?>/id/' + id,
			success: function(data) {
				$('#id').val(data.id)
				$('#nama-barang').html('<b>'+nama_barang+'</b>')
				$('#jumlah-edit').val(data.masuk)
				$('#ed-edit').val(datefmysql(data.ed))
	...
	function updateStokOpname() {
		$.ajax({
			type: 'PUT',
			url: '<?php echo base_url('stok_opname/api/stok_opname/update_stok_opname') ?>',
	...
	function hapusStokOpname(id, page) {
		swal.fire({
			...
			if (result.value) {
				$.ajax({
					type: 'DELETE',
					url: '<?php echo base_url('stok_opname/api/stok_opname/delete_stok_opname') ?>/id/' + id,
					success: function(data) {
						let page = $('#page').val()
						if (data.status) {
