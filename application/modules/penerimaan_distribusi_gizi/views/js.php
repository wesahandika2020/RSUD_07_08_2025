<script>
	$(function() {
		...
		$('#no-distribusi-search').select2({
            width: '100%',
            ajax: {
                url: "<?= base_url('inventori_gizi/api/inventori_gizi_auto/nomor_distribusi_auto') ?>",
                dataType: 'json',
                quietMillis: 100,
		...
		$('#no-distribusi-auto').select2c({
            ...
            formatSelection: function(data) {
				if (data.id !== '') {
					$.ajax({
						type: 'GET',
						url: '<?php echo base_url('distribusi_gizi/api/distribusi_gizi/get_data_distribusi_gizi') ?>/id/' + data.id,
    ...
    function getListPenerimaanDistribusi(page) {
		...
		$.ajax({
			type: 'GET',
			url: '<?php echo base_url('penerimaan_distribusi_gizi/api/penerimaan_distribusi_gizi/get_list_penerimaan_distribusi') ?>/page/' + page,
    ...
    function simpanPenerimaanDistribusi() {
        ...
        swal.fire({
			...
			if (result.value) {
				$.ajax({
                    type: 'POST',
                    url: '<?php echo base_url('penerimaan_distribusi_gizi/api/penerimaan_distribusi_gizi/simpan_penerimaan_distribusi') ?>',
    ...
    function deletePenerimaanDistribusi(id) {
        swal.fire({
			...
			if (result.value) {
				$.ajax({
                    type: 'DELETE',
                    url: '<?php echo base_url('penerimaan_distribusi_gizi/api/penerimaan_distribusi_gizi/delete_penerimaan_distribusi') ?>/id/' + id,
                    dataType:'JSON',
                    beforeSend: function() {
                        showLoader()
                    },
