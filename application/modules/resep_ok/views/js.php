<script>
	...
	function getListResep(p, id_layanan_pendaftaran) {
		...
		$.ajax({
			type: 'GET',
			url: '<?= base_url("resep_ok/api/resep_ok/get_list_resep_ok") ?>/page/' + p + '/jenis/OK Central/id/' + id,
			data: $('#form-search').serialize() + '&keyword=' + $('#keyword').val(),
			cache: false,
			dataType: 'JSON',
			beforeSend: function() {
				showLoader();
			...
			success: function(data) {
				...
				$.each(data.data, function(i, v) {
					...
					let detail = v.id + '#' + v.id_pasien + '#' + v.nama + '#' + v.dokter + '#' + v.id_dokter + '#' + hitungUmur(v.tanggal_lahir) + '#' + v.layanan + '#' + v.id_penjamin + '#' + v.penjamin + '#' + v.cara_bayar + '#' + v.telp + '#ok';
					...
					html = /* html */ `
                        <tr>
                            ...
                            <td>${(v.layanan_ok == 'OK' ? 'OK Central' : '')}</td>
	...
	function cetakBuktiPelayanan(id_resep, pengambilan_ke) {
		...
		window.open('<?= base_url('resep/printing_bukti_pelayanan_obat') ?>/' + id_resep + '/' + pengambilan_ke, 'Cetak Bukti Resep OK Central', 'width=' + dWidth + ', height=' + dHeight + ', left=' + x + ',top=' + y);
	}
