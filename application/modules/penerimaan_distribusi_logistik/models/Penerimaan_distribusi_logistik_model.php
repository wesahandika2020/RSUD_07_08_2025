...
class Penerimaan_distribusi_logistik_model extends CI_Model {
	...
	private function sqlPenerimaanDistribusi($search)
    {
		...
		");
		$this->db->from('sm_penerimaan_distribusi_logistik as pd');
		$this->db->join('sm_invrt_distribusi as d', 'd.id = pd.id_distribusi');
		$this->db->join('sm_invrt_detail_distribusi as dd', 'dd.id_distribusi = d.id', 'left');
	...
	function simpanDataPenerimaanDistribusi() 
	{
		...
		$this->db->insert('sm_penerimaan_distribusi_logistik', $data);
	...
	{
		$this->db->trans_begin();
		$this->db->where(['id' => $id])->delete('sm_penerimaan_distribusi_logistik');
		$this->db->where(['id_transaksi' => $id, 'transaksi' => 'Penerimaan Distribusi'])->delete('sm_invrt_stok');
		if ($this->db->trans_status() === false) :
			$this->db->trans_rollback();
			$result['status'] = false;
