...
<div id="pemakaian-gizi-search" class="modal fade" data-backdrop="static" role="dialog" aria-labelledby="pemakaian-gizi-search-label" aria-hidden="true">
	<div class="modal-dialog" style="max-width:500px">
		<div class="modal-content">
			<div class="modal-header">
				<h4 class="modal-title" id="pemakaian-gizi-search-label">Form Parameter Laporan</h4>
				<button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
			</div>
			<div class="modal-body">
				<?= form_open('', 'id=form-search-pemakaian-gizi role=form class=form-horizontal') ?>
				...
				<div class="form-group row tight">
					<label for="barang-pemakaian-gizi" class="col-md-3 col-form-label">Barang</label>
					<div class="col-md-9">
						<input type="text" name="barang" id="barang-pemakaian-gizi" class="select2-input" placeholder="Semua Barang">
					</div>
				</div>
				<?= form_close() ?>
			</div>
			<div class="modal-footer">
				...
				<button type="button" class="btn btn-info waves-effect" onclick="getLaporanPemakaianGizi(1)"><i class="fas fa-paper-plane mr-1"></i> Tampilkan</button>
			</div>
		</div>
		<!-- /.modal-content -->
	</div>
...
<script>
	...
	function resetFormPemakaiangizi() {
		<?php date_default_timezone_set('Asia/Jakarta'); ?>
	...
	function getLaporanPemakaianGizi(page) {
		//Laporan
		$('#page-now').val(page)
		...
		if ($('#jenis-laporan').val() == "5") {
			$('.lap-pemakaian-gizi, #table-lap-pemakaian-gizi tbody').show()

			$.ajax({
				type: 'GET',
				url: '<?= base_url('laporan_inventori_gizi/api/laporan_inventori_gizi/get_list_laporan_pemakaian') ?>/page/' + page + '/jenis/',
				data: $('#form-search-pemakaian-gizi').serialize(),
				cache: false,
				dataType: 'JSON',
				beforeSend: function() {
					showLoader()
				...
				success: function(data) {
					if ((page - 1) & (data.data.length === 0)) {
						getLaporanPemakaianGizi(page - 1);
						return false;
					}

					$('#jenis-periode-pemakaian-gizi').html('Periode : ' + data.periode);
					...
					$.each(data.data, function(i, v) {
						...
						let periode = '';
						if ($('#jenis-periode-pemakaian-gizi').val() == 'Harian') {
							periode = 'Laporan Harian';
						}
						if ($('#jenis-periode-pemakaian-gizi').val() == 'Bulanan') {
							periode = 'Laporan Bulanan';
						}
						if ($('#jenis-periode-pemakaian-gizi').val() == 'Rentang Waktu') {
						...
						$('#table-lap-pemakaian-gizi tbody').append(html);
					})
				},
				complete: function() {
					hideLoader()
	...
	$('#barang-pemakaian-gizi').select2({
		ajax: {
			url: "<?= base_url('laporan_inventori_gizi/api/laporan_inventori_gizi/barang_non_kategori') ?>",
			dataType: 'json',
			quietMillis: 100,
