<!-- Modal Search -->
<div id="pengeluaran-gizi-search" class="modal fade" data-backdrop="static" role="dialog" aria-labelledby="pengeluaran-gizi-search-label" aria-hidden="true">
	<div class="modal-dialog" style="max-width:500px">
		<div class="modal-content">
			<div class="modal-header">
				<h4 class="modal-title" id="pengeluaran-gizi-search-label">Form Parameter Laporan</h4>
				<button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
			</div>
			<div class="modal-body">
				<?= form_open('', 'id=form-search-pengeluaran-gizi role=form class=form-horizontal') ?>
			...
			<div class="modal-footer">
				...
				<button type="button" class="btn btn-info waves-effect" onclick="getLaporanPengeluaranGizi(1)"><i class="fas fa-paper-plane mr-1"></i> Tampilkan</button>
			</div>
		</div>
		<!-- /.modal-content -->
	</div>
...
<script>
	...
	function resetForm_kel() {
		...
		$('#kategori-kel, #jenis-pasien, #unit-depo-kel').val('')
	...
	function getLaporanPengeluaranGizi(page) {
		$('#page-now').val(page)
		$('#pengeluaran-gizi-search').modal('hide')
		openData();

		if ($('#jenis-laporan').val() == "2") {
			$('.lap-pengeluaran-gizi, #table-lap-pengeluaran-gizi tbody').show()

			$.ajax({
				type: 'GET',
				url: '<?= base_url('laporan_inventori_gizi/api/laporan_inventori_gizi/get_list_laporan_pengeluaran') ?>/page/' + page + '/jenis/',
				data: $('#form-search-pengeluaran-gizi').serialize(),
				cache: false,
				dataType: 'JSON',
				beforeSend: function() {
					showLoader()
				...
				success: function(data) {
					if ((page - 1) & (data.data.length === 0)) {
						getLaporanPengeluaranGizi(page - 1);
						return false;
					}

					$('#jenis-periode-pengeluaran-gizi').html('Periode : ' + data.periode);
					...
					$.each(data.data, function(i, v) {
						...
						let periode = '';
						if ($('#jenis-periode-pengeluaran-gizi').val() == 'Harian') {
							periode = 'Laporan Harian';
						}
						if ($('#jenis-periode-pengeluaran-gizi').val() == 'Bulanan') {
							periode = 'Laporan Bulanan';
						}
						if ($('#jenis-periode-pengeluaran-gizi').val() == 'Rentang Waktu') {
					...
					$('#table-lap-pengeluaran-gizi tfoot').append(html);
					// })
				},
				complete: function() {
					hideLoader()
