...
class Laporan_inventori_gizi_model extends CI_Model
{
    ...
    //     $unit_condition = $search["unit"] !== "" 
    //         ? " AND bu.id_unit = ?" 
    //         : " AND bu.id_unit = ?";
    //     $unit_value = $search["unit"] !== "" 
    //         ? $search["unit"] 
    //         : $this->session->userdata('id_unit');

    //     $kategori_condition = $search["kategori"] !== "" 
    //         ? " AND b.id_kategori = ?" 
    //         : "";
    //     $kategori_value = $search["kategori"] !== "" 
    //         ? $search["kategori"] 
    //         : null;
    ...
    //             0 AS saldo_awal_unit, /* Placeholder */
    //             0 AS saldo_awal_rp, /* Placeholder */
    //             COALESCE(SUM(sgs.masuk), 0) AS penerimaan_unit,
    //             COALESCE(SUM(sgs.masuk) * sgdp.harga, 0) AS penerimaan_rp,
    //             COALESCE(SUM(sgs.keluar), 0) AS pengeluaran_unit,
    //             COALESCE(SUM(sgs.keluar) * sgdp.harga, 0) AS pengeluaran_rp,
    //             0 AS saldo_akhir_unit, /* Placeholder */
    //             0 AS saldo_akhir_rp, /* Placeholder */
    //             '' AS keterangan
    ...
    //         LEFT JOIN sm_gizi_stok AS sgs ON sgs.id_barang = b.id
    ...
    //     // Perhitungan stok awal berdasarkan periode laporan
    //     foreach ($result as $i => $val) {
    //         $param_stok_awal = "";

    //         if ($search["periode_laporan"] === "Harian" && $search["tanggal_harian"] !== "") {
    //             $param_stok_awal = " AND to_char(waktu, 'yyyy-mm-dd') < '" . date2mysql($search['tanggal_harian']) . "'";
    //         } elseif ($search["periode_laporan"] === "Bulanan" && $search["bulan"] !== "" && $search["tahun"] !== "") {
    //             $param_stok_awal = " AND to_char(waktu, 'yyyy-mm') < '" . $search["tahun"] . "-" . $search["bulan"] . "'";
    //         } elseif ($search["periode_laporan"] === "Rentang Waktu" && $search["tanggal_awal"] !== "" && $search["tanggal_akhir"] !== "") {
    //             $param_stok_awal = " AND waktu < '" . date2mysql($search['tanggal_awal']) . " 00:00:00'";
    //         }

    //         // Query untuk stok awal
    //         $stok_awal_query = "
    ...
    //         $stok_awal = $this->db->query($stok_awal_query, [$val->id_barang])->row()->awal;

    //         // Hitung saldo awal dan akhir
    //         $result[$i]->saldo_awal_unit = $stok_awal;
    //         $result[$i]->saldo_awal_rp = $stok_awal * $val->harga_satuan;

    //         $result[$i]->saldo_akhir_unit = $stok_awal + $result[$i]->penerimaan_unit - $result[$i]->pengeluaran_unit;
