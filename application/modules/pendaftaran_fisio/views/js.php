<script>
	$(function() {
        ...
        $('#btn-tambah-data').click(function() {
            ...
            $('#domisili').val('');
            $('#btn-search-nik').hide();
            $('#no-identitas').attr('readonly', true);
            $('.reset-field, .reset-select, #cara_bayar').attr('disabled', true);
            $('#modal-pendaftaran').modal('show');
            $('#modal-pendaftaran-label').html('Form Entri Pendaftaran Fisioterapi');
		});
            ...
            $.ajax({
                ...
                success: function(data) {
                    if (data.data.success === true) {
                        ...
                        $('#nama, #kelamin, #tanggal-lahir, #tempat-lahir, #alamat, #provinsi, #kabupaten, #kecamatan, #kelurahan, #agama, #goldarah, #pekerjaan, #pendidikan, #pernikahan, #ayah, #ibu').attr('readonly', true);
                        messageCustom(data.data.message, 'Success');
                    } else if (data.data.success === false) {
                        swalCustom('warning', 'Informasi NIK', data.data.message);
                        resetAllForm();
                        $('#no-identitas').attr('readonly', false).focus()
                    }
                },
                complete: function() {
                    hideLoader();
        ...
        $('#domisili').change(function() {
            if ($('#domisili').val() == 1) {
                ...
                $('#asal-masuk').val(2);
                $('#bt-search-nik').show();
                $('#no-identitas').attr('readonly', false);
                $('#provinsi, #kabupaten').attr('readonly', true);
                ...
                getKecamatan(36, 71);
                syams_validation_remove(this);
                $('.reset-field, .reset-select, #cara-bayar').attr('disabled', false);
            } else {
                resetAllForm();
                $('#asal-masuk').val(2);
                $('#bt-search-nik').hide();
                $('#no-identitas').attr('readonly', false);
                getProvinsi();
                $('#nama, #kelamin, #tanggal-lahir, #tempat-lahir, #alamat, #provinsi, #kabupaten, #kecamatan, #kelurahan, #agama, #pernikahan, #ayah, #ibu').attr('readonly', false);
                syams_validation_remove(this);
                $('.reset-field, .reset-select, #cara-bayar').attr('disabled', false);
            }
        });
	...
	function resetAllForm() {
        ...
        $('#nama, #kelamin, #tanggal-lahir, #tempat-lahir, #alamat, #agama, #pernikahan, #ayah, #ibu').attr('readonly', false);
	...
	function getListPemeriksaan(page, id) {
        ...
        $.ajax({
            type: 'GET',
            url: '<?= base_url('pendaftaran/api/pendaftaran/get_list_pendaftaran') ?>/page/' + page + '/jenis_pendaftaran/Fisioterapi',
    ...
    function konfirmasiSimpanDataPendaftaran() {
		...
		klik = null;
		swal.fire({
			title: 'Simpan Pendaftaran Fisioterapi',
			text: "Apakah anda yakin akan melakukan entri pendaftaran Fisioterapi ?",
			icon: 'question',
                    ...
                    if (data.layanan.length > 0) {
                        $.each(data.layanan, function(i, v){
                            if (i === 0) {
                                ...
                                $('#dokter-fisio').val(v.id_dokter);
                                $('#s2id_dokter-fisio a .select2-chosen').html(v.dokter);
                            };
                        });

                        if ($('#id-layanan').val() !== '') {
                            getPemeriksaanFisio($('#id-layanan').val());    
                        };

                        $('#modal-detail-pendaftaran').modal('show');
                        $('#modal-detail-pendaftaran-label').html('Detail Pendaftaran Fisioterapi');
                    }       
