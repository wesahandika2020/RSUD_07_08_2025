<script>
	...
	$(function() {
		...
		// resep tab
		$('#resep-tab button:first').tab('show');
	...
	function inputResep(detail, form) {
		...
		if (data[6] === 'Intensive Care' | data[6] === 'Laboratorium' | data[6] === 'Radiologi' | data[6] === 'Fisioterapi' | data[6] === 'Hemodialisa') {
			$('#salin-resep').hide();
		} else {
			$('#salin-resep').show();
		}
		...
		var titleModal = ''
		if (data[6] === 'Intensive Care') {
			titleModal = 'Intensive Care';
		...
		$('#modal-form-resep-label').html(`
            <b>Form Resep Intensive Care</b> | No. RM: ${data[1]}, Nama: ${data[2]}, 
	...
	function addToResep() {
		...
		$('.barang').each(function(i) {
			if (barang === $(this).val()) {
				allow = false;
				swalAlert('warning', 'Validasi', 'Barang <b>' + nama_barang + '</b> sudah di input kan pada baris No. <b>' + (i + 1) + '</b>');
				return false;
			}
		});
	...
	function getListHistoryResep(p, id_resep) {
		...
		$.ajax({
			...
			data: $('#form-search-resep').serialize() + '&pasien=' + $('#pasienhide').val(),
			cache: false,
			dataType: 'JSON',
			beforeSend: function() {
				showLoader();
			...
			success: function(data) {
				...
				$('#table-data-resep-icare tbody').empty();
				let no = '';
				let num = 1;
				let html = '';
				$.each(data.data, function(i, v) {
					html = /* html */ `
                        <tr>
                            ...
                            <td>${v.dokter}</td>
                            <td>${v.jenis_layanan} <b>${(v.layanan_ok == 'OK' ? '<i class="fas fa-arrow-circle-right mr-1"></i>OK Central' : '')}</b></td>
                            <td>${v.no_r}</td>
                            <td>${v.nama_barang}</td>
					...
					$('#table-data-resep-icare tbody').append(html);
					if (no !== v.id) {
						num++;
					}
					no = v.id;
	...
	function simpanDataResep() {
		...
		if (klik === null) {
			klik = $.ajax({
				...
				data: $('#form-add-resep').serialize() + `&is_terapi_pulang=${isTerapiPulang}&obat_pulang=`,
				cache: false,
				dataType: 'JSON',
				beforeSend: function() {
					showLoader();
