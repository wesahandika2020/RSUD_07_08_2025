<script>
	...
	function entriPengkajianAwal(id_pendaftaran, id_layanan_pendaftaran, bed) {
		$('#wizard_form_pengkajian_awal_ranap').bwizard('show', '0');	
		$.ajax({
			type: 'GET',
			url: '<?= base_url("rawat_inap/api/rawat_inap/get_data_pengkajian_awal_ranap") ?>/id/' + id_pendaftaran + '/id_layanan/' + id_layanan_pendaftaran,
			cache: false,
			dataType: 'JSON',
			beforeSend: function() {
				showLoader()
			...
			success: function(data) {
				...
				$('#modal_pengkajian_awal').modal('show');
			},
			complete: function() {
				hideLoader()
			},
	...
	function showKajianPerawat(data) {
		...
		if (kbs.produktif !== '') { $('#bs_produktif').prop('checked', true) }
		if (kbs.non_produktif !== '') { $('#bs_non_produktif').prop('checked', true) }
		if (kbs.hemaptoe !== '') { $('#bs_hemaptoe').prop('checked', true) }
		if (kbs.lain !== '') { 
		...
		if (data.reproduksi_alat === 'Hemoroid') { $('#sr_genital_hemoroid').prop('checked', true).change() }
		if (data.reproduksi_alat === 'Hipospadia') { $('#sr_genital_hipospadia').prop('checked', true).change() }
		if (data.reproduksi_alat === 'Masalah Prostat') { $('#sr_genital_masalah_prostat').prop('checked', true).change() }
		if (data.reproduksi_alat === 'Simetris') { $('#sr_genital_simetris').prop('checked', true).change() }
		if (data.reproduksi_alat === 'Asimetris') { $('#sr_genital_asimetris').prop('checked', true).change() }
		if (data.reproduksi_alat === 'Inflamasi') { $('#sr_genital_inflamasi').prop('checked', true).change() }
		if (data.reproduksi_alat === 'Nyeri') { $('#sr_genital_nyeri').prop('checked', true).change() }
	...
	function konfirmasiSimpanPengkajianAwal() {
		// console.log($('#form_pengkajian_awal').serialize());
		var stop = false;
	...
	function simpanPengkajianAwal() {
		...
		$.ajax({
			...
			success: function(data) {
				...
				$('#modal_pengkajian_awal').modal('hide');
			},
...
<!-- Modal -->
<!-- Rawat Inap -->
<div class="modal fade" id="modal_pengkajian_awal" data-backdrop="static">
	<div class="modal-dialog modal-dialog-scrollable" style="max-width: 90%;">
		<div class="modal-content">
			<div class="modal-header">
				<div class="title">
					<h5 class="modal-title bold" id="modal_pengkajian_awal">FORM PENGKAJIAN AWAL PASIEN RAWAT INAP</h5>
			...
			<div class="modal-footer">
				...
				<button type="button" class="btn btn-info" onclick="konfirmasiSimpanPengkajianAwal()" id="btn_simpan"><i class="fas fa-fw fa-save mr-1"></i>Konfirmasi</button>
			</div>
		</div>
	</div>
</div>
