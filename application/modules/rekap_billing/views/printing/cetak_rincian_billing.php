...
<script>
	...
	function simpanWatuCetak() {
		let id_pendaftaran = "<?= $id_pendaftaran ?>";
		$.ajax({
			type: 'POST',
			url: '<?= base_url('rekap_billing/api/rekap_billing/simpan_waktu_cetak') ?>',
			data: 'id_pendaftaran=' + id_pendaftaran + '&keterangan=1',
			cache: false,
			dataType: 'JSON',
			success: function(data) {
				if (!data.status) {
					alert('Gagal simpan waktu cetak id=' + id_pendaftaran)
				}
			},
			error: function(e) {
				alert('Gagal simpan waktu cetak id==' + id_pendaftaran)
...
<body onload="cetak()">
	<div class="page">
		...
		<?php foreach ($layanan as $key => $lay) : ?>
			...
			<table width="100%" class="tabel-laporan">
				...
				<tbody>
					...
					<!-- PKRT-->
					<?php if (count((array)$lay->pkrt) > 0) : ?>
						<tr>
							<td colspan="6"><b style="font-size:14px;font-weight:bold;">PKRT (Perbekalan Kesehatan Rumah Tangga)</b></td>
						</tr>
						<?php
						$subtotal = 0;
						$id_lay = NULL;
						...
						$print = true;
						?>
						<?php foreach ($lay->pkrt as $key => $value) : ?>
							<?php
							$subtotal += $value->total;
							$qty = $value->qty;
							$total_pkrt = $value->total;
							$print = true;
							// endif;
							if ($key === (count((array)$lay->pkrt) - 1)) :
								$print = true;
							endif;
							?>
							<?php if ($print) : ?>
								<tr>
									<td style="padding-left:15px;"><?= $value->item ?></td>
									<td class="center" style="white-space:nowrap;"><?= ($value->waktu !== '' ? '(' . datetimefmysql($value->waktu, true) . ')&nbsp;&nbsp;' : '') ?></td>
									<td class="center"></td>
									<td class="center"><?= $value->qty ?></td>
									<td class="right"><?= currency($value->harga_item) ?></td>
									<td class="right"><?= currency($total_pkrt) ?></td>
								</tr>
							<?php endif; ?>
						<?php endforeach; ?>
						<tr>
		...
		<table width="100%">
			<tr>
				<td width="40%" class="left">( <?= $pasien->nama ?> )</td>
				<?php if (!empty($_SESSION['account_group']) ?? "") : ?>
			...
			<tr>
				<td width="40%" class="left"><?= $waktu ?></td>
				<td width="40%" class="right"><?= $waktu ?></td>
			</tr>
