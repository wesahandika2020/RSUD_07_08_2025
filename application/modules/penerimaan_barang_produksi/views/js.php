<script>
	$(function() {
		...
		$('#no-distribusi-auto').select2c({
			width: '100%',
			ajax: {
				url: "<?= base_url('inventory/api/inventory_auto/nomor_distribusi_auto') ?>",
				dataType: 'json',
				quietMillis: 100,
			...
			formatSelection: function(data) {
				if (data.id !== '') {
					$.ajax({
						type: 'GET',
						url: '<?php echo base_url('distribusi/api/distribusi/get_data_distribusi') ?>/id/' + data.id,
	...
	function simpanPenerimaanDistribusi() {
		...
		swal.fire({
			...
			if (result.value) {
				$.ajax({
					type: 'POST',
					url: '<?php echo base_url('penerimaan_barang_produksi/api/penerimaan_barang_produksi/simpan_penerimaan_distribusi_barang') ?>',
	...
	function getListPenerimaanDistribusi(page) {
		...
		$.ajax({
			type: 'GET',
			url: '<?php echo base_url('penerimaan_barang_produksi/api/penerimaan_barang_produksi/get_list_penerimaan_distribusi') ?>/page/' + page,
	...
	function  deletePenerimaanDistribusi(id) {
		swal.fire({
			...
			if (result.value) {
				$.ajax({
					type: 'DELETE',
					url: '<?php echo base_url('penerimaan_barang_produksi/api/penerimaan_barang_produksi/delete_penerimaan_distribusi') ?>/id/' + id,
					dataType:'JSON',
					beforeSend: function() {
						showLoader()
					},
