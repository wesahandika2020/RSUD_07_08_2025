...
class Penerimaan_barang_produksi_model extends CI_Model
{
	...
	private function sqlPenerimaanProduksi($search)
	{
		...
		");
		$this->db->from('sm_penerimaan_distribusi as pd');
		$this->db->join('sm_distribusi as d', 'd.id = pd.id_distribusi');
		$this->db->join('sm_detail_distribusi as dd', 'dd.id_distribusi = d.id', 'left');
	...
	private function _listPenerimaanProduksi($limit = 0, $start = 0, $search)
	{
		$this->sqlPenerimaanProduksi($search);
		if ($limit !== 0) :
	...
	function getListDataPenerimaanProduksi($limit = 0, $start = 0, $search)
	{
		$result['data'] = $this->_listPenerimaanProduksi($limit, $start, $search);
		$result['jumlah'] = $this->sqlPenerimaanProduksi($search)->get()->num_rows();
		return $result;
	...
	function simpanDataPenerimaanDistribusi()
	{
		...
		$this->db->insert('sm_penerimaan_distribusi', $data);
		...
		if (is_array($stoks)) :
			foreach ($stoks as $data) :
				$ket_unit = $this->m_inventory->namaUnit($data->id_unit);
				$data_stok = array(
					'waktu' => safe_post('waktu_penerimaan') !== '' ? datetime2mysql(safe_post('waktu_penerimaan')) : $this->datetime,
					'id_transaksi' => $id,
	...
	{
		$this->db->trans_begin();
		$this->db->where(['id' => $id])->delete('sm_penerimaan_distribusi');
		$this->db->where(['id_transaksi' => $id, 'transaksi' => 'Penerimaan Distribusi'])->delete('sm_stok');
		if ($this->db->trans_status() === false) :
			$this->db->trans_rollback();
			$result['status'] = false;
