...
<body>
  ...
  <div style="text-align: center;">
    <h4 style="text-transform: uppercase;">LAPORAN KASIR (REKAPITULASI)
      <br>Periode : <?= $periode ?>
    </h4>
  </div>
    ...
    <tbody>
      ...
      ?>
        <tr>
          ...
          <td class="left"><?= "Rp. " . number_format($v->total_obat, 0, ",", ".") ?></td>
          <td class="left"><?= "Rp. " . number_format($v->total_tindakan, 0, ",", ".") ?></td>
        </tr>
      <?php endforeach ?>
    </tbody>
    <tfoot>
      <tr>
        ...
        <?php foreach ($data as $v) : ?>
          ...
          <?php $sum_obat += $v->total_obat; ?>
          <?php $sum_tindakan += $v->total_tindakan; ?>
        <?php endforeach ?>
        <td class="left"><?= "Rp. " . number_format($total_tagihan, 0, ",", ".") ?></td>
        <td class="left"><?= "Rp. " . number_format($sum_obat, 0, ",", ".") ?></td>
        <td class="left"><?= "Rp. " . number_format($sum_tindakan, 0, ",", ".") ?></td>
      </tr>
    </tfoot>
  </table>
</body>
