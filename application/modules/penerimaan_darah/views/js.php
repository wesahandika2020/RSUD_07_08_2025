<script>
	...
	$(function() {
		...
		$('#btn-add').click(function() {
			reloadData()
			$('#modal-penerimaan-title').html('Form Penerimaan Barang Bank Darah')
	...
	function getListPenerimaan(page, id) {
		...
		$.ajax({
			type: 'GET',
			url: '<?= base_url("inventory/api/inventory/get_list_penerimaan_bank_darah") ?>/page/' + page + '/id/' + id,
			data: $('#form-search').serialize() + '&jenis_barang=Bank Darah',
			cache: false,
			dataType: 'JSON',
			beforeSend: function() {
				showLoader()
				...
				$.each(data.data, function(i, v) {
					...
					let html = /* html */ `
						<tr>
							...
							<td class="wrapper right">${numberToCurrency(grandTotal)}</td>
	...
	function printPenerimaan(id_penerimaan) {
		window.open('<?= base_url() ?>inventory/printing_penerimaan_bank_darah/' + id_penerimaan, 'Cetak Penerimaan','width='+dWidth+', height='+dHeight+', left='+x+',top='+y)
	}
	...
	function deletePenerimaan(id, page, string) {
		...
		swal.fire({
			...
			if (result.value) {
				$.ajax({
					type: 'DELETE',
					url: '<?= base_url("inventory/api/inventory/delete_penerimaan_bank_darah") ?>' + '/id/' + id,
					cache: false,
					dataType: 'JSON',
					beforeSend: function() {
						showLoader()
	...
	function simpanPenerimaan() {
		...
		$.ajax({
			type: 'POST',
			url: '<?= base_url("inventory/api/inventory/simpan_penerimaan_bank_darah") ?>' + update,
	...
	function showDetailPenerimaan(id_penerimaan) {
		$.ajax({
			type: 'GET',
			url: '<?= base_url('inventory/api/inventory/get_detail_penerimaan_bank_darah') ?>/id/' + id_penerimaan,
			cache: false,
			dataType: 'JSON',
			beforeSend: function() {
				showLoader()
