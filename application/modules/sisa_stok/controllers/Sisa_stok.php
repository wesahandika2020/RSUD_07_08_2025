...
class Sisa_stok extends SYAM_Controller
{
    ...
    function __construct()
    {
		...
		$this->load->model('Sisa_stok_model', 'm_stok');
    }
	...
	function page_sisa_stok() {
		...
		$data['periode_laporan'] = $this->m_stok->getPeriodeLaporan();
		$data['bulan']           = $this->m_masterdata->getBulan();
		$this->load->view( 'sisa_stok/index', $data );
	}

	public function export_sisa_stok_unit() {
		$search = [
		...
		$data                    = $this->m_stok->getListDataSisaStok( 0, 0, $search );
		...
		$this->load->view( 'export/sisa_stok_unit.php', $data );
	}
}
