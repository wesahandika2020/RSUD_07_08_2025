 ...
 */
class Pembayaran_lain extends REST_Controller
{
	...
	function simpan_pembayaran_lain_post()
	{
		...
		if ($unit_kasir !== '') :
			...
			if ($bayar !== '') :
				$data = array(
					...
					'no_kwitansi' 				=> $this->m_pembayaran_lain->generateNoKwitansiPembayaran($unit_kasir),
				...
				$response = $this->m_pembayaran_lain->simpanPembayaranLain($data);
			endif;
		else :
			$status = false;
	...
	function get_list_pembayaran_lain_get()
	{
		if (!$this->get('page')) :
		...
		$data            = $this->m_pembayaran_lain->getListDataPembayaranLain($this->limit, $start, $search);
